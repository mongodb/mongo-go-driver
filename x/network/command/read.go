...
func (r *Read) createReadPref(serverKind description.ServerKind, topologyKind description.TopologyKind) bsonx.Doc {
	doc := bsonx.Doc{}
	rp := r.ReadPref

	if serverKind == description.Mongos && rp != nil {
		if rp.Mode() == readpref.PrimaryMode {
			return nil
		} else if rp.Mode() == readpref.SecondaryPreferredMode {
			if _, ok := r.ReadPref.MaxStaleness(); !ok && len(r.ReadPref.TagSets()) == 0 {
				return nil
			}
		}
	} else if topologyKind == description.Single {
...
func (r *Read) addReadPref(rp *readpref.ReadPref, serverKind description.ServerKind, topologyKind description.TopologyKind, query bson.Raw) (bson.Raw, error) {
	doc := r.createReadPref(serverKind, topologyKind)
	if doc == nil {
		return query, nil
	}
...
func (r *Read) encodeOpMsg(desc description.SelectedServer, cmd bsonx.Doc) (wiremessage.WireMessage, error) {
	...
	readPrefDoc := r.createReadPref(desc.Server.Kind, desc.Kind)
