...
var connectDirectOpMsgTests = []struct {
	...
	{description.Mongos, readpref.Primary(), readpref.Mode(uint8(0))},
...
var connectDirectOpQueryTests = []struct {
	...
	{description.Mongos, readpref.Primary(), readpref.Mode(uint8(0)), 0},
	{description.Mongos, readpref.SecondaryPreferred(), readpref.Mode(uint8(0)), wiremessage.SlaveOK},
...
func TestCommandEncode(t *testing.T) {
	for _, tt := range connectDirectOpMsgTests {
		t.Run("connect direct op_msg", func(t *testing.T) {
			...
			if tt.expectedReadPrefMode == readpref.Mode(uint8(0)) {
				if err == nil {
				...
				} else if mode, _ := readpref.ModeFromString(rp.StringValue()); mode != tt.expectedReadPrefMode {
