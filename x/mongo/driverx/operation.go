...
type OperationContext struct {
	...
	ServerSelector func(context.Context) (Server, error)
...
func (oc OperationContext) SelectServer(ctx context.Context) (Server, error) {
	...
	if oc.ServerSelector != nil {
		return oc.ServerSelector(ctx)
	}
...
func (oc OperationContext) Validate() error {
	...
	if (oc.Deployment == nil) && (oc.ServerSelector == nil && oc.TopologyKind == description.TopologyKind(0)) {
		return errors.New("the Deployment field or the ServerSelector and TopologyKind fields must be set on OperationContext")
	}
	if oc.Database == "" {
...
func (oc OperationContext) Execute(ctx context.Context) error {
	...
	if oc.Deployment != nil {
		kind = oc.Deployment.Description().Kind
	}
