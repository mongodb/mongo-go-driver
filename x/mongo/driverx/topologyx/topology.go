...
func (t *Topology) SelectServer(ctx context.Context, ss description.ServerSelector) (driverx.Server, error) {
	if atomic.LoadInt32(&t.connectionstate) != connected {
...
func (t *Topology) FindServer(selected description.Server) (*Server, error) {
	if atomic.LoadInt32(&t.connectionstate) != connected {
		return nil, ErrTopologyClosed
	}
	t.serversLock.Lock()
...
func (t *Topology) update() {
	...
	for {
		...
		case change := <-t.changes:
			// TODO(GODRIVER-617): This context.TODO could stall out topology updates if there is an
			// outstanding connection and it doesn't get returned to the pool for some reason.
...
func (t *Topology) removeServer(ctx context.Context, addr address.Address, server *Server) {
	// TODO(GODRIVER-617): Don't take a context.Context for this method. If we're removing the
	// server we want to aggressively close it immediately, so we should provide server.Disconnect
	// with a cancelled context.Context.
