...
type connection struct {
	...
	// TODO(GODRIVER-617): Add reference to topologyx.Server
	p          *pool
...
func (c *connection) ReadWireMessage(ctx context.Context, dst []byte) ([]byte, error) {
	...
	// TODO(GODRIVER-617): We used to call ProcessWriteConcernError in driver, but we should handle
	// that here instead. It should be fast since we're using bsoncore (and we could potentially
	// optimize with the bytes package).
	return wm, nil
}
