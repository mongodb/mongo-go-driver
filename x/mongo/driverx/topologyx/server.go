...
func NewServer(addr address.Address, opts ...ServerOption) (*Server, error) {
	...
	s.pool, err = NewPool(addr, uint64(cfg.maxIdleConns), maxConns, cfg.connectionOpts...)
	// s.pool, err = NewPool(addr, uint64(cfg.maxIdleConns), maxConns)
	if err != nil {
		return nil, err
	}
...
func (s *Server) Connection(ctx context.Context) (driverx.Connection, error) {
...
func (s *Server) updateDescription(desc description.Server, initial bool) {
	...
	// TODO(GODRIVER-617): This should just be done inside of the update method.
	switch desc.Kind {
...
func (s *Server) heartbeat(conn *connection) (description.Server, *connection) {
	...
	for i := 1; i <= maxRetry; i++ {
		...
		if conn == nil {
			...
			conn, err = newConnection(ctx, s.pool, s.address, opts...)
			if err != nil {
				saved = err
				if conn != nil {
					conn.Close()
		...
		for i, comp := range s.cfg.compressionOpts {
		...
		isMasterCmd := commandx.IsMaster{Compressors: bsoncore.Value{Type: bsontype.Array, Data: compressors}}
