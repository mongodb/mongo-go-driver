...
func NewBatchCursor(result bsoncore.Document, clientSession *session.Client, clock *session.ClusterClock, server Server, maxTimeMS, batchSize *int64) (*BatchCursor, error) {
	...
	bc.options.maxTimeMS = maxTimeMS
	bc.options.batchSize = batchSize
	...
	for _, elem := range elems {
		...
		case "ns":
			...
			bc.namespace.Collection = namespace.Collection
			bc.namespace.DB = namespace.DB
		case "id":
...
// provided parameters.
func NewLegacyBatchCursor(ns Namespace, cursorID int64, ds *bsoncore.DocumentSequence, limit int32, batchSize int32, server Server) (*BatchCursor, error) {
...
func (bc *BatchCursor) Close(ctx context.Context) error {
	...
	// if bc.legacy() {
	// 	// TODO(GODRIVER-617): We should have KillCursorsOperation handle both legacy and non-legacy
	// 	// commands.
