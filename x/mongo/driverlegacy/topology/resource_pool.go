...
func NewResourcePool(maxSize uint64, expiredFn expiredFunc, closeFn closeFunc, cleanupInterval time.Duration) *resourcePool {
	...
	rp.cleanupTimer = time.AfterFunc(rp.cleanupInterval, rp.Prune)
	return rp
}
...
func (rp *resourcePool) Put(v interface{}) bool {
	...
	rp.prune()
	if rp.maxSize != 0 && rp.len == rp.maxSize {
		return false
	}
...
func (rp *resourcePool) prune() {
	...
	rp.cleanupTimer.Stop()
	rp.cleanupTimer.Reset(rp.cleanupInterval)
}
