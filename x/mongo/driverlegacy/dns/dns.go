...
func (r *Resolver) ParseHosts(host string) ([]string, error) {
	...
	return r.fetchSeedlistFromSRV(parsedHosts[0])
}
...
func (r *Resolver) fetchSeedlistFromSRV(host string) ([]string, error) {
	...
	for _, address := range addresses {
		...
		currentErr := validateSRVResult(trimmedAddressTarget, host)
		if currentErr != nil {
			err = currentErr
			continue
		}
		parsedHosts[currIndex] = fmt.Sprintf("%s:%d", trimmedAddressTarget, address.Port)
		currIndex++
	}
	return parsedHosts[:currIndex], err
}
