...
func DefaultResolver() Resolver {
	return Resolver{net.LookupSRV, net.LookupTXT}
}

// ParseHosts uses the srv string to get the hosts.
func (r *Resolver) ParseHosts(host string) ([]string, error) {
...
func validateSRVResult(recordFromSRV, inputHostName string) error {
	...
	if len(separatedRecord) < 2 {
		return fmt.Errorf("DNS name must contain at least 2 labels: %s", recordFromSRV)
	}
	if len(separatedRecord) < len(separatedInputDomain) {
		return fmt.Errorf("Domain suffix from SRV record not matched input domain: %s", recordFromSRV)
	}
