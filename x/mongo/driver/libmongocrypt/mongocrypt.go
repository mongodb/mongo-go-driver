...
import (
	...
	"go.mongodb.org/mongo-driver/x/mongo/driver/libmongocrypt/options"
)

// KmsProvider represents a provider for a KMS.
type KmsProvider string

// These constants reprsent valid values for KmsProvider.
const (
	AwsProvider   KmsProvider = "aws"
	LocalProvider KmsProvider = "local"
)
...
// CreateEncryptionContext creates a MongoCryptContext to use for encryption.
func (m *MongoCrypt) CreateEncryptionContext(db string, cmd bsoncore.Document) (*MongoCryptContext, error) {
	ctx := newMongoCryptContext(C.mongocrypt_ctx_new(m.wrapped))
	if ctx.wrapped == nil {
...
// CreateDecryptionContext creates a MongoCryptContext to use for decryption.
func (m *MongoCrypt) CreateDecryptionContext(cmd bsoncore.Document) (*MongoCryptContext, error) {
	ctx := newMongoCryptContext(C.mongocrypt_ctx_new(m.wrapped))
	if ctx.wrapped == nil {
		return nil, m.createErrorFromStatus()
	}
...
// CreateDataKeyContext creates a MongoCryptContext to use for creating a data key.
func (m *MongoCrypt) CreateDataKeyContext(kmsProvider KmsProvider, opts *options.DataKeyOptions) (*MongoCryptContext, error) {
	ctx := newMongoCryptContext(C.mongocrypt_ctx_new(m.wrapped))
	if ctx.wrapped == nil {
		return nil, m.createErrorFromStatus()
	}
...
// CreateExplicitEncryptionContext creates a MongoCryptContext to use for explicit encryption.
func (m *MongoCrypt) CreateExplicitEncryptionContext(doc bsoncore.Document, algorithm string,
	opts *options.ExplicitEncryptionOptions) (*MongoCryptContext, error) {
...
// CreateExplicitDecryptionContext creates a MongoCryptContext to use for explicit decryption.
func (m *MongoCrypt) CreateExplicitDecryptionContext(doc bsoncore.Document) (*MongoCryptContext, error) {
	ctx := newMongoCryptContext(C.mongocrypt_ctx_new(m.wrapped))
	if ctx.wrapped == nil {
		return nil, m.createErrorFromStatus()
	}
...
// setLocalProviderOpts sets options for the local KMS provider in libmongocrypt.
...
// setAwsProviderOpts sets options for the AWS KMS provider in libmongocrypt.
...
// setLocalSchemaMap sets the local schema map in libmongocrypt.
