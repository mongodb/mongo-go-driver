...
const resourcesDir = "./test_resources"
...
func testKmsCtx(t *testing.T, ctx *MongoCryptContext, keyAltName bool) {
	...
	kmsCtx = ctx.NextKmsContext()
	if kmsCtx != nil {
		t.Fatalf("expected nil but got a MongoCryptKmsContext")
	}
...
func TestMongoCrypt(t *testing.T) {
	t.Run("encrypt", func(t *testing.T) {
		...
		t.Run("invalid bson", func(t *testing.T) {
			...
			if _, ok := err.(MongoCryptError); !ok {
				t.Fatalf("error type mismatch; expected MongoCryptError, got %v", err)
			}
		})
	})
