...
// PostBatchResumeToken returns the current post batch resume token, if one is available.
...
func (bc *BatchCursor) getMore(ctx context.Context) {
	...
	// 4.2+ look for post batch resume token
	if pbrt, err := response.LookupErr("postBatchResumeToken"); err == nil && pbrt.Type == bsontype.EmbeddedDocument {
		bc.postBatchResumeToken = bsoncore.Document(pbrt.Document())
	}
...
func (bc *BatchCursor) legacy() bool {
	return bc.server.Description().WireVersion.Max < 4
}
