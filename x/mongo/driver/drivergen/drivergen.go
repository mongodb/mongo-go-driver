...
func (op Operation) CommandMethod() (string, error) {
	...
	for name, field := range op.Request {
		if op.Properties.Batches != nil && name == op.Properties.Batches.Identifier {
			continue
		}
		var tmpl *template.Template
		switch field.Type {
...
func (p Properties) IsEnabled(builtin string) Builtin {
	...
	case Deployment, Database, Selector:
	default:
	}
	return ""
}
...
// Batches are the configuration information for batch splitting operations.
type Batches struct {
	Identifier string
	Documents  string
	Ordered    string
