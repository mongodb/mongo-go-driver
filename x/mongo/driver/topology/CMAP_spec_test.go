...
type testInfo struct {
	...
	originalEventChan      chan PoolEvent
	finalEventChan         chan PoolEvent
...
func runCMAPTest(t *testing.T, testFileName string) {
	...
	testInfo := &testInfo{
		...
		originalEventChan:      make(chan PoolEvent, 200),
		finalEventChan:         make(chan PoolEvent, 200),
	...
	pc := poolConfig{
		MaxPoolSize: uint64(test.PoolOptions.MaxPoolSize),
		MinPoolSize: uint64(test.PoolOptions.MinPoolSize),
		MaxIdleTime: time.Duration(test.PoolOptions.MaxIdleTimeMS * 1000000),
		PoolMonitor: func(event PoolEvent) {
			testInfo.originalEventChan <- event
		},
		Address: address.Address(l.Addr().String()),
	}

	p, err := newPool(pc)
	testHelpers.RequireNil(t, err, "error creating connection pool: %v", err)
	err = p.Connect()
	testHelpers.RequireNil(t, err, "error connecting connection pool %v", err)

	for _, op := range test.Operations {
		if tempErr := runOperation(t, op, testInfo, p, test.PoolOptions.WaitQueueTimeoutMS); tempErr != nil {
			if err != nil {
...
func checkEvents(t *testing.T, expectedEvents []cmapEvent, actualEvents chan PoolEvent, ignoreEvents []string) {
...
func nextValidEvent(t *testing.T, events chan PoolEvent, ignoreEvents []string) PoolEvent {
	t.Helper()
NextEvent:
	for {
		if len(events) == 0 {
...
func runOperation(t *testing.T, operation map[string]interface{}, testInfo *testInfo, p *pool, checkOutTimeout int32) error {
	...
	if ok { // to be run in background thread
		...
		thread.JobQueue <- func() {
			err := runOperationInThread(t, operation, testInfo, p, checkOutTimeout)
			testInfo.backgroundThreadErrors <- err
		}

		return nil // since we don't care about errors occurring in non primary threads
	}
	return runOperationInThread(t, operation, testInfo, p, checkOutTimeout)
}

func runOperationInThread(t *testing.T, operation map[string]interface{}, testInfo *testInfo, p *pool, checkOutTimeout int32) error {
	...
	case "wait":
		...
		dur := time.Duration(int64(timeMs.(float64) * 1000000))
		time.Sleep(dur)
	case "waitForThread":
	...
	case "checkOut":
		...
		if checkOutTimeout != 0 {
			...
			checkoutContext, cancel = context.WithTimeout(context.Background(), time.Duration(checkOutTimeout*1000000))
			defer cancel()
		}

		c, err := p.CheckOut(checkoutContext)
