...
func TestResourcePool(t *testing.T) {
	t.Run("Get", func(t *testing.T) {
		t.Run("remove stale resources", func(t *testing.T) {
			...
			rp := initPool(t, 0, 1, 0, ec.expired, ec.close, initRsrc, time.Minute)
		...
		t.Run("recycle resources", func(t *testing.T) {
			rp := initPool(t, 0, 1, 0, neverExpired, closeRsrc, initRsrc, time.Minute)
			rp.maintainTimer.Stop()
			for i := 0; i < 5; i++ {
				got := rp.Get()
				if got == nil {
	...
	t.Run("Prune", func(t *testing.T) {
		t.Run("removes all stale resources", func(t *testing.T) {
			...
			rp := initPool(t, 0, 0, 0, ec.expired, ec.close, initRsrc, time.Minute)
			for i := 0; i < 5; i++ {
				ret := &rsrc{}
				_ = rp.Return(ret)
			}
	...
	t.Run("Background cleanup", func(t *testing.T) {
		t.Run("runs once every interval", func(t *testing.T) {
			...
			rp := initPool(t, 0, 0, 0, neverExpired, ec.close, initRsrc, dur)
			rp.maintainTimer.Stop()
