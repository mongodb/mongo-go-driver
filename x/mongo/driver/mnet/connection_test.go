...
//	getHandshakeInformation func(context.Context, address.Address, *Connection) (HandshakeInformation, error)
//	finishHandshake         func(context.Context, *Connection) error
//}
//
//// GetHandshakeInformation implements the Handshaker interface.
//func (th *testHandshaker) GetHandshakeInformation(
//	ctx context.Context,
//	addr address.Address,
//	conn *Connection,
//) (HandshakeInformation, error) {
//	if th.getHandshakeInformation != nil {
//		return th.getHandshakeInformation(ctx, addr, conn)
//	}
//	return HandshakeInformation{}, nil
//}
//
//// FinishHandshake implements the Handshaker interface.
//func (th *testHandshaker) FinishHandshake(ctx context.Context, conn *Connection) error {
...
//}
//
//var _ Handshaker = &testHandshaker{}
//
...
//
//			opts := Options{
//				Dialer: DialerFunc(func(context.Context, string, string) (net.Conn, error) { return nil, dialError }),
//			}
//
//			conn := NewConnection(address.Address(""), opts)
//
//			err := conn.unsafe.Connect(context.Background())
//			assert.True(t, errors.Is(err, dialError), "expected error %v, got %v", dialError, err)
//		})
//
//		t.Run("handshake error", func(t *testing.T) {
//			conn := NewConnection(address.Address(""), Options{
//				Dialer:     DialerFunc(func(context.Context, string, string) (net.Conn, error) { return &net.TCPConn{}, nil }),
//				Handshaker: &testHandshaker{},
//			})
//
//			defer conn.unsafe.Close()
//
//			ctx, cancel := context.WithCancel(context.Background())
//			cancel()
//			err := conn.unsafe.Connect(ctx)
...
//				conn := NewConnection(address.Address(""), nil)
//				wantTimeout := time.Duration(0)
//				assert.Equal(t, wantTimeout, conn.unsafe.IdleTimeout, "expected idle timeout %v, got %v", wantTimeout,
//					conn.unsafe.IdleTimeout)
//			})
//		})
//		t.Run("connect", func(t *testing.T) {
//			t.Run("dialer error", func(t *testing.T) {
//				err := errors.New("dialer error")
//				var want error = Error{Wrapped: err, init: true}
//
//				conn := NewConnection(address.Address(""), &Options{
//					Dialer: DialerFunc(func(context.Context, string, string) (net.Conn, error) { return nil, err }),
//				})
//
//				got := conn.unsafe.Connect(context.Background())
//				if !cmp.Equal(got, want, cmp.Comparer(compareErrors)) {
//					t.Errorf("errors do not match. got %v; want %v", got, want)
//				}
//
//				connState := atomic.LoadInt64(&conn.unsafe.State)
//				assert.Equal(t, driverutil.ConnDisconnected, connState,
//					"expected connection state %v, got %v", driverutil.ConnDisconnected, connState)
//			})
...
//					assert.Nil(t, conn.unsafe.CancelConnectContext, "cancellation function was not cleared")
//				})
