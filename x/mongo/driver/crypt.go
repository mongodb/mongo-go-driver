...
// Crypt consumes the libmongocyrpt.MongoCrypt type to iterate the mongocrypt state machine and perform encryption
// and decryption.
type Crypt struct {
...
// EncryptExplicitly encrypts the given value with the given options.
func (c *Crypt) EncryptExplicitly(ctx context.Context, val bsoncore.Value, opts *options.ExplicitEncryptionOptions) (byte, []byte, error) {
...
// DecryptExplicitly decrypts the given encrypted value.
func (c *Crypt) DecryptExplicitly(ctx context.Context, subtype byte, data []byte) (bsoncore.Value, error) {
	idx, doc := bsoncore.AppendDocumentStart(nil)
