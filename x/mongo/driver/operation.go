...
type Operation struct {
	...
	// MinimumWriteConcernWireVersion specifies the maximum wire version to add the write concern to
	// the command being executed.
...
func (op Operation) addReadConcern(dst []byte, desc description.SelectedServer) ([]byte, error) {
	if op.MinimumReadConcernWireVersion > 0 && (desc.WireVersion == nil || !desc.WireVersion.Includes(int32(op.MinimumReadConcernWireVersion))) {
		return dst, nil
	}
	rc := op.ReadConcern
	client := op.Client
...
func (op Operation) addWriteConcern(dst []byte, desc description.SelectedServer) ([]byte, error) {
	if op.MinimumWriteConcernWireVersion > 0 && (desc.WireVersion == nil || !desc.WireVersion.Includes(int32(op.MinimumWriteConcernWireVersion))) {
		return dst, nil
	}
	wc := op.WriteConcern
	if wc == nil {
