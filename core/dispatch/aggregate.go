...
func Aggregate(
	...
	if aggOpts.AllowDiskUse != nil {
		cmd.Opts = append(cmd.Opts, bsonx.Elem{"allowDiskUse", bson.Boolean(*aggOpts.AllowDiskUse)})
	}
	if aggOpts.BatchSize != nil {
		elem := bsonx.Elem{"batchSize", bson.Int32(*aggOpts.BatchSize)}
	...
	if aggOpts.BypassDocumentValidation != nil && desc.WireVersion.Includes(4) {
		cmd.Opts = append(cmd.Opts, bsonx.Elem{"bypassDocumentValidation", bson.Boolean(*aggOpts.BypassDocumentValidation)})
	}
	...
	if aggOpts.MaxTime != nil {
		cmd.Opts = append(cmd.Opts, bsonx.Elem{"maxTimeMS", bson.Int64(int64(*aggOpts.MaxTime / time.Millisecond))})
	}
	...
	if aggOpts.Comment != nil {
		cmd.Opts = append(cmd.Opts, bsonx.Elem{"comment", bson.String(*aggOpts.Comment)})
	}
	if aggOpts.Hint != nil {
