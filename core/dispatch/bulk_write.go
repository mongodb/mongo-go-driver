...
var CollationError = errors.New("collation cannot be set for server versions < 3.4")
var ArrayFiltersError = errors.New("array filters cannot be set for server versions < 3.6")
...
func verifyOptions(models []WriteModel, ss *topology.SelectedServer) error {
	...
	for _, model := range models {
		...
		if afSet && maxVersion < 6 {
			return ArrayFiltersError
		}

		if collationSet && maxVersion < 5 {
			return CollationError
		}
	}
...
func createBatches(models []WriteModel, ordered bool) []bulkWriteBatch {
	...
	for _, batch := range batches {
		batch.canRetry = true // default retry to true
	}
