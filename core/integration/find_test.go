...
func initChannels() {
	startedChan = make(chan *event.CommandStartedEvent, 10)
	succeededChan = make(chan *event.CommandSucceededEvent, 10)
	failedChan = make(chan *event.CommandFailedEvent, 10)
}
...
func TestFindPassesMaxAwaitTimeMSThroughToGetMore(t *testing.T) {
	...
	if id < 5 {
		t.Errorf("not all documents returned; last seen id = %d", id)
	}
}
