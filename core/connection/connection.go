...
func (s *snappyCompressor) uncompressBytes(compressed []byte) ([]byte, error) {
	var err error
	s.uncompressBuf, err = snappy.Decode(s.uncompressBuf, compressed)

	if err != nil {
		return []byte{}, err
	}

	return s.uncompressBuf, nil
}
...
type connection struct {
	...
	id               string
	conn             net.Conn
	compressor       Compressor                              // use for compressing messages
	compressorMap    map[wiremessage.CompressorID]Compressor // use for uncompressing messages
	dead             bool
...
func createSnappy() *snappyCompressor {
...
func createZlib(level int) (*zlibCompressor, error) {
	if level < 0 {
...
func New(ctx context.Context, addr address.Address, opts ...Option) (Connection, *description.Server, error) {
	...
	connSnappy := createSnappy()
	connZlib, err := createZlib(-1)
	...
	c := &connection{
		id:   id,
		conn: nc,
		compressorMap: map[wiremessage.CompressorID]Compressor{
			wiremessage.CompressorSnappy: connSnappy,
			wiremessage.CompressorZLib:   connZlib,
		},
		addr:             addr,
		...
		clientMethodLoop:
			for _, method := range d.ClientCompression {
				for _, serverMethod := range d.Compression {
					...
					switch method {
					case "snappy":
						c.compressor = createSnappy()
					case "zlib":
						c.compressor, err = createZlib(c.zlibLevel)
						if err != nil {
							return nil, nil, err
						}
					}
...
func (c *connection) uncompressMessage(compressed wiremessage.Compressed) ([]byte, wiremessage.OpCode, error) {
	...
	uncompressedMessage, err := uncompressor.uncompressBytes(compressed.CompressedMessage)
