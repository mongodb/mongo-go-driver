...
import (
	...
	"github.com/mongodb/mongo-go-driver/core/monitor"
...
type connection struct {
	...
	commandMap       map[int64]*monitor.CmdMetadata // map for monitoring commands sent to server
	dead             bool
	idleTimeout      time.Duration
	idleDeadline     time.Time
	lifetimeDeadline time.Time
	cmdMonitor       *monitor.CmdMonitor
...
func New(ctx context.Context, addr address.Address, opts ...Option) (Connection, *description.Server, error) {
	...
	c := &connection{
		...
		commandMap: make(map[int64]*monitor.CmdMetadata),
		addr:             addr,
...
func (c *connection) commandStartedEvent(wm wiremessage.WireMessage) error {
	...
	event := &monitor.CommandStartedEvent{
		ConnectionID: c.id,
	}
	...
	case wiremessage.Query:
		...
		event.DatabaseName = converted.FullCollectionName[:len(converted.FullCollectionName)-5] // remove $.cmd
		event.RequestID = int64(converted.MsgHeader.RequestID)
	case wiremessage.Msg:
		...
		event.DatabaseName = dbVal.StringValue()
		event.RequestID = int64(converted.MsgHeader.RequestID)
	}

	event.Command = cmd
	event.CommandName = event.Command.ElementAt(0).Key()
	if !canMonitor(event.CommandName) {
		event.Command = emptyDoc
	}

	c.cmdMonitor.Started(event)
	c.commandMap[event.RequestID] = monitor.CreateMetadata(event.CommandName)
	return nil
}
...
func (c *connection) commandFinishedEvent(wm wiremessage.WireMessage) error {
	...
	finishedEvent := monitor.CommandFinishedEvent{
	...
	if success {
		...
		event := &monitor.CommandSucceededEvent{
			Reply:                reply,
			CommandFinishedEvent: finishedEvent,
		}

		c.cmdMonitor.Succeeded(event)
		return nil
	}

	event := &monitor.CommandFailedEvent{
		Failure:              errmsg,
		CommandFinishedEvent: finishedEvent,
	}

	c.cmdMonitor.Failed(event)
	return nil
}
