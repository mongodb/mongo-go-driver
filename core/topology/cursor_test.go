...
func TestTailableCursorLoopsUntilDocAvailable(t *testing.T) {
	...
	c := cursor{
		...
		batch:  bson.NewArray(),
		server: s,
		opts: []option.CursorOptioner{option.OptCursorType(option.Tailable)},
	}

	assert.True(t, c.Next(nil))
}

func TestTailableCursorReturnsFalseOnContextCancellation(t *testing.T) {
