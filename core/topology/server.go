...
// Disconnect closes sockets to the server referenced by this Server. It will
// shutdown any monitoring goroutines, close the idle connection pool, and will
...
func (s *Server) Subscribe() (*ServerSubscription, error) {
	if atomic.LoadInt32(&s.connectionstate) == 0 {
