...
func (r *Read) createReadPref(kind description.ServerKind) bsonx.Doc {
	...
	case readpref.PrimaryMode:
		doc = append(doc, bsonx.Elem{"mode", bson.String("primary")})
	case readpref.PrimaryPreferredMode:
		doc = append(doc, bsonx.Elem{"mode", bson.String("primaryPreferred")})
	case readpref.SecondaryPreferredMode:
		doc = append(doc, bsonx.Elem{"mode", bson.String("secondaryPreferred")})
	case readpref.SecondaryMode:
		doc = append(doc, bsonx.Elem{"mode", bson.String("secondary")})
	case readpref.NearestMode:
		doc = append(doc, bsonx.Elem{"mode", bson.String("nearest")})
	}
	...
	for _, ts := range r.ReadPref.TagSets() {
		...
		set := bsonx.Doc{}
		for _, t := range ts {
			set = append(set, bsonx.Elem{t.Name, bson.String(t.Value)})
		}
	...
	if d, ok := r.ReadPref.MaxStaleness(); ok {
		doc = append(doc, bsonx.Elem{"maxStalenessSeconds", bson.Int32(int32(d.Seconds()))})
	}
