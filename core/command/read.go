...
// Encode r as OP_MSG
func (r *Read) encodeOpMsg(desc description.SelectedServer) (wiremessage.WireMessage, error) {
	msg := wiremessage.Msg{
	...
	fullDocRdr, err := opmsgAddGlobals(r.Command, r.DB, readPrefDoc)
	if err != nil {
		return nil, err
	}
...
func (r *Read) addReadConcern() error {
	if r.ReadConcern == nil {
		return nil
	}

	element, err := r.ReadConcern.MarshalBSONElement()
	if err != nil {
		return err
	}

	if _, err := r.Command.LookupElementErr(element.Key()); err == bson.ErrElementNotFound {
		r.Command.Append(element)
	}
	return nil
}
...
func (r *Read) encodeOpQuery(desc description.SelectedServer) (wiremessage.WireMessage, error) {
	rdr, err := marshalCommand(r.Command)
	if err != nil {
		return nil, err
	}
...
func (r *Read) RoundTrip(ctx context.Context, desc description.SelectedServer, rw wiremessage.ReadWriteCloser) (bson.Reader, error) {
	defer rw.Close()
