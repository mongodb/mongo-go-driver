...
func (r *Read) Encode(desc description.SelectedServer) (wiremessage.WireMessage, error) {
	...
	addClusterTime(cmd, desc, r.Session, r.Clock)
...
func (r *Read) Decode(desc description.SelectedServer, wm wiremessage.WireMessage) *Read {
	...
	clusterTime := responseClusterTime(r.result)
	if clusterTime != nil {
		if r.Session != nil {
			r.Session.AdvanceClusterTime(clusterTime)
		}

		r.Clock.AdvanceClusterTime(clusterTime)
	}
