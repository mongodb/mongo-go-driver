...
func (h *Handshake) Encode() (wiremessage.WireMessage, error) {
	var wms wiremessage.WireMessage
...
// are called.
func (h *Handshake) Decode(wms wiremessage.WireMessage) *Handshake {
	h.ismstr, h.err = (&IsMaster{}).Decode(wms).Result()
	if h.err != nil {
		return h
	}
	return h
...
func (h *Handshake) Handshake(ctx context.Context, addr address.Address, rw wiremessage.ReadWriter) (description.Server, error) {
	...
	err = rw.WriteWireMessage(ctx, wms)
	if err != nil {
		return description.Server{}, err
	}

	wms, err = rw.ReadWireMessage(ctx)
	if err != nil {
		return description.Server{}, err
	}
