...
func updateClusterTimes(sess *session.Client, clock *session.ClusterClock, response bson.Reader) {
	...
	if sess != nil {
		sess.AdvanceClusterTime(clusterTime)
	}
...
func addSessionID(cmd *bson.Document, desc description.SelectedServer, client *session.Client) error {
	if client == nil || desc.WireVersion.Max < session.MinWireVersion || desc.SessionTimeoutMinutes == 0 {
		return nil
	}
...
func addClusterTime(cmd *bson.Document, desc description.SelectedServer, sess *session.Client, clock *session.ClusterClock) {
	if (clock == nil && sess == nil) || desc.WireVersion.Max < session.MinWireVersion {
		return
	}
	...
	cmd.Append(bson.EC.SubDocument("$clusterTime", clusterTime))
	return
}
