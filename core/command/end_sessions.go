...
createBatches:
	for {
		for i := 0; i < BatchSize; i++ {
			...
			batches[outerIndex] = append(batches[outerIndex], es.SessionIDs[docIndex])
			docIndex++
		}
...
func (es *EndSessions) encodeBatch(batch []*bson.Document, desc description.SelectedServer) *Write {
	...
	if es.ClusterTime != nil {
		cmd.Append(bson.EC.SubDocument("$clusterTime", es.ClusterTime))
	}

	return &Write{
		ClusterTime: es.ClusterTime,
		DB:          "admin",
		Command:     cmd,
	}
}
