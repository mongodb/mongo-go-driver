...
func (dc *DropCollection) decode(desc description.SelectedServer, rdr bson.Reader) *DropCollection {
	dc.resultClusterTime = responseClusterTime(rdr)
	if dc.Session != nil {
		dc.Session.AdvanceClusterTime(dc.resultClusterTime)
	}
...
func (dc *DropCollection) Result() (bson.Reader, *bson.Document, error) {
	if dc.err != nil {
		return nil, nil, dc.err
	}
	return dc.result, dc.resultClusterTime, nil
}
...
func (dc *DropCollection) RoundTrip(ctx context.Context, desc description.SelectedServer, rw wiremessage.ReadWriter) (bson.Reader, *bson.Document, error) {
	cmd, err := dc.encode(desc)
	if err != nil {
		return nil, nil, err
	}
