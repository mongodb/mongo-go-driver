...
func (lc *ListCollections) decode(desc description.SelectedServer, cb CursorBuilder, rdr bson.Reader) *ListCollections {
	lc.resultClusterTime = responseClusterTime(rdr)
	if lc.Session != nil {
		lc.Session.AdvanceClusterTime(lc.resultClusterTime)
	}
	...
	lc.result, lc.err = cb.BuildCursor(rdr, lc.Session, opts...)
