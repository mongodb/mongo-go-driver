...
func (c *scramSaslClient) Start() (string, []byte, error) {
	if err := c.generateClientNonce(scramSHA1NonceLen); err != nil {
		return SCRAMSHA1, nil, &Error{"", err}
	}
...
func (c *scramSaslClient) step1(challenge []byte) ([]byte, error) {
	...
	if len(fields) != 3 {
		return nil, &Error{"invalid server response", nil}
	}

	if !bytes.HasPrefix(fields[0], []byte("r=")) || len(fields[0]) < 2 {
		return nil, &Error{"invalid nonce", nil}
	}
	r := fields[0][2:]
	...
	if !bytes.HasPrefix(fields[1], []byte("s=")) || len(fields[1]) < 6 {
		return nil, &Error{"invalid salt", nil}
	}
	...
	if !bytes.HasPrefix(fields[2], []byte("i=")) || len(fields[2]) < 3 {
		return nil, &Error{"invalid iteration count", nil}
	}
...
func (c *scramSaslClient) step2(challenge []byte) ([]byte, error) {
	...
	if hasE {
		return nil, &Error{string(fields[0][2:]), nil}
	}
	if !hasV {
		return nil, &Error{"invalid final message", nil}
	}

	v := make([]byte, base64.StdEncoding.DecodedLen(len(fields[0][2:])))
	n, err := base64.StdEncoding.Decode(v, fields[0][2:])
	if err != nil {
		return nil, &Error{"invalid server verification", nil}
	}
	v = v[:n]

	if !bytes.Equal(c.serverSignature, v) {
		return nil, &Error{"invalid server signature", nil}
	}
