...
func Handshaker(appName string, h connection.Handshaker, authenticator Authenticator, compressors []string, zlibLevel int) connection.Handshaker {
	return connection.HandshakerFunc(func(ctx context.Context, addr address.Address, rw wiremessage.ReadWriter) (description.Server, error) {
		desc, err := (&command.Handshake{Client: command.ClientDoc(appName), Compressors: compressors, ZlibLevel: zlibLevel}).Handshake(ctx, addr, rw)
		if err != nil {
