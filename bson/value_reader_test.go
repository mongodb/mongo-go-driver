...
func TestValueReader(t *testing.T) {
	...
	t.Run("ReadBinary", func(t *testing.T) {
		...
		t.Run("success", func(t *testing.T) {
			...
			vr := &valueReader{
				...
				stack: []vrState{
					{mode: vrTopLevel},
					{mode: vrElement, vType: TypeCodeWithScope},
				},
				frame: 1,
			}
	...
	t.Run("Skip", func(t *testing.T) {
		docb, err := NewDocument(EC.Null("foobar")).MarshalBSON()
		noerr(t, err)
		...
		testCases := []struct {
			...
			{"DBPointer/not enough bytes", TypeDateTime, []byte{0x01, 0x02, 0x03, 0x04}, io.EOF, 0},
			{"DBPointer/success", TypeDateTime, []byte{0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08}, nil, 8},
			{"Double/not enough bytes", TypeDouble, []byte{0x01, 0x02, 0x03, 0x04}, io.EOF, 0},
			{"Double/success", TypeDouble, []byte{0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08}, nil, 8},
			{"Int64/not enough bytes", TypeInt64, []byte{0x01, 0x02, 0x03, 0x04}, io.EOF, 0},
			{"Int64/success", TypeInt64, []byte{0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08}, nil, 8},
			{"Timestamp/not enough bytes", TypeTimestamp, []byte{0x01, 0x02, 0x03, 0x04}, io.EOF, 0},
			{"Timestamp/success", TypeTimestamp, []byte{0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08}, nil, 8},
			{
			...
			{"Int32/not enough bytes", TypeInt32, []byte{0x01, 0x02}, io.EOF, 0},
			{"Int32/success", TypeInt32, []byte{0x01, 0x02, 0x03, 0x04}, nil, 4},
			{"Javascript/invalid length", TypeJavaScript, strbytes[:2], io.EOF, 0},
			{"Javascript/not enough bytes", TypeJavaScript, strbytes[:5], io.EOF, 4},
			{"Javascript/success", TypeJavaScript, strbytes, nil, 8},
			{"String/invalid length", TypeString, strbytes[:2], io.EOF, 0},
			{"String/not enough bytes", TypeString, strbytes[:5], io.EOF, 4},
			{"String/success", TypeString, strbytes, nil, 8},
			{"Symbol/invalid length", TypeSymbol, strbytes[:2], io.EOF, 0},
			{"Symbol/not enough bytes", TypeSymbol, strbytes[:5], io.EOF, 4},
			{"Symbol/success", TypeSymbol, strbytes, nil, 8},
			{"MaxKey/success", TypeMaxKey, []byte{0x01}, nil, 0},
			{"MinKey/success", TypeMinKey, []byte{0x01}, nil, 0},
			{"Null/success", TypeNull, []byte{0x01}, nil, 0},
			{"Undefined/success", TypeUndefined, []byte{0x01}, nil, 0},
			{
			...
			},
			{
				...
				Type(0),
				nil,
				fmt.Errorf("attempted to skip unknown BSON type %v", Type(0)), 0,
			},
		}
	...
	t.Run("invalid transition", func(t *testing.T) {
		vr := &valueReader{stack: []vrState{{mode: vrTopLevel}}}
		wanterr := (&valueReader{stack: []vrState{{mode: vrTopLevel}}}).invalidTransitionErr(0)
		goterr := vr.Skip()
