...
func (ejp *extJSONParser) peekType() (bson.Type, error) {
	var t bson.Type
	var err error
	...
	case jpsSawBeginObject:
		...
		case jpsSawEndObject: // empty embedded document
			t = bson.TypeEmbeddedDocument
		case jpsInvalidState:
			err = ejp.err
		case jpsSawKey:
			ejp.vIsDBRef = ejp.k == "$ref" || ejp.k == "$id"

			t = wrapperKeyBSONType(ejp.k)

			if t == bson.TypeJavaScript {
				// just saw $code, need to check for $scope at same level
				_, err := ejp.readValue(bson.TypeJavaScript)
...
func (ejp *extJSONParser) readKey() (string, bson.Type, error) {
	...
	// check DBRef (not a bson type) key constraints
	if (key == "$ref" || key == "$db") && ejp.vIsDBRef && t != bson.TypeString {
		return "", 0, invalidJSONError(fmt.Sprintf("string for DBRef field %s", key))
	}
...
func (ejp *extJSONParser) readValue(t bson.Type) (*extJSONValue, error) {
	...
	switch t {
	case bson.TypeNull, bson.TypeBoolean, bson.TypeString:
	...
	case bson.TypeInt32, bson.TypeInt64, bson.TypeDouble:
		...
		fallthrough
	case bson.TypeDecimal128, bson.TypeSymbol, bson.TypeObjectID, bson.TypeMinKey, bson.TypeMaxKey, bson.TypeUndefined:
		switch ejp.s {
		case jpsSawKey:
			// read colon
			ejp.advanceState()
	...
	case bson.TypeBinary, bson.TypeRegex, bson.TypeTimestamp, bson.TypeDBPointer:
		if ejp.s != jpsSawKey {
			return nil, invalidRequestError(t.String())
		}
		// read colon
		...
		v = &extJSONValue{t: bson.TypeEmbeddedDocument, v: &extJSONObject{keys: keys, values: vals}}
...
func (ejp *extJSONParser) ensureExtValueType(t bson.Type) bool {
	switch t {
	case bson.TypeMinKey, bson.TypeMaxKey:
		return ejp.v.t == bson.TypeInt32
	case bson.TypeUndefined:
		return ejp.v.t == bson.TypeBoolean
	case bson.TypeInt32, bson.TypeInt64, bson.TypeDouble, bson.TypeDecimal128, bson.TypeSymbol, bson.TypeObjectID:
		return ejp.v.t == bson.TypeString
	default:
		return false
	}
}
...
func invalidJSONErrorForType(expected string, t bson.Type) error {
