...
func (js *jsonScanner) nextToken(expectKey bool) (*jsonToken, error) {
	...
	case '"', '\'':
		return js.scanString(c)
	default:
		// if expecting a key, treat this as an unquoted string
		if expectKey {
...
// scanString reads until the argued terminator and handles escaped characters
func (js *jsonScanner) scanString(term byte) (*jsonToken, error) {
	var b bytes.Buffer
	var c byte
	var err error
	...
	// for unquoted strings, include the first character
	if term == ':' {
		b.WriteByte(js.buf[p])
	}
	...
	for {
		...
		case term:
			// for unquoted strings, ':' is the terminator
			if term == ':' {
				// the scanner should return the ':' on the next call to nextToken()
				js.pos--
			}
