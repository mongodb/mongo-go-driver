...
func (ejvw *extJSONValueWriter) WriteCodeWithScope(code string) (DocumentWriter, error) {
	...
	err := writeStringWithEscapes(code, &buf, ejvw.escapeHTML)
	if err != nil {
		return nil, err
	}
...
func (ejvw *extJSONValueWriter) WriteRegex(pattern string, options string) error {
	...
	err := writeStringWithEscapes(pattern, &buf, ejvw.escapeHTML)
	if err != nil {
		return err
	}
...
func (ejvw *extJSONValueWriter) WriteString(s string) error {
	...
	var buf bytes.Buffer
	err := writeStringWithEscapes(s, &buf, ejvw.escapeHTML)
	if err != nil {
		return err
	}
...
func (ejvw *extJSONValueWriter) WriteSymbol(symbol string) error {
	...
	var buf bytes.Buffer
	err := writeStringWithEscapes(symbol, &buf, ejvw.escapeHTML)
	if err != nil {
		return err
	}
...
func writeStringWithEscapes(s string, buf *bytes.Buffer, escapeHTML bool) error {
	if escapeHTML {
		b := new(bytes.Buffer)
