...
func (ejvw *extJSONValueWriter) reset(buf []byte, canonical bool) {
	if ejvw.stack == nil {
...
func (ejvw *extJSONValueWriter) WriteCodeWithScope(code string) (DocumentWriter, error) {
	...
	writeStringWithEscapes(code, &buf)
...
func (ejvw *extJSONValueWriter) WriteJavascript(code string) error {
	...
	ejvw.writeExtendedSingleValue("code", buf.String(), true)
	ejvw.buf = append(ejvw.buf, ',')
...
func (ejvw *extJSONValueWriter) WriteRegex(pattern string, options string) error {
	...
	writeStringWithEscapes(pattern, &buf)
...
func (ejvw *extJSONValueWriter) WriteSymbol(symbol string) error {
	...
	var buf bytes.Buffer
	writeStringWithEscapes(symbol, &buf)

	ejvw.writeExtendedSingleValue("symbol", buf.String(), true)
	ejvw.buf = append(ejvw.buf, ',')
...
func writeStringWithEscapes(s string, buf *bytes.Buffer) {
	for _, c := range s {
		switch c {
		case '"':
			buf.WriteString(`\"`)
