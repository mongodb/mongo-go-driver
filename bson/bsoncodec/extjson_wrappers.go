...
func wrapperKeyBSONType(key string) bson.Type {
	switch string(key) {
	case "$numberInt":
	...
	case "$symbol":
		return bson.TypeSymbol
	case "$numberDouble":
		return bson.TypeDouble
	case "$numberDecimal":
		return bson.TypeDecimal128
	case "$binary":
		return bson.TypeBinary
	case "$code":
		return bson.TypeJavaScript
	case "$scope":
		return bson.TypeCodeWithScope
	case "$timestamp":
		return bson.TypeTimestamp
	case "$regularExpression":
		return bson.TypeRegex
	case "$dbPointer":
		return bson.TypeDBPointer
	case "$date":
	...
	case "$id":
		fallthrough
	case "$db":
		return bson.TypeEmbeddedDocument // TODO: dbrefs aren't bson types
	case "$minKey":
		return bson.TypeMinKey
	case "$maxKey":
		return bson.TypeMaxKey
	case "$undefined":
		return bson.TypeUndefined
	}
...
func (ejv *extJSONValue) parseBinary() (b []byte, subType byte, err error) {
	if ejv.t != bson.TypeEmbeddedDocument {
