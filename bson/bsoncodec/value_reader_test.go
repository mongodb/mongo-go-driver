...
func TestValueReader(t *testing.T) {
	...
	t.Run("Skip", func(t *testing.T) {
		...
		testCases := []struct {
			...
			{"Javascript/not enough bytes", bson.TypeJavaScript, strbytes[:5], io.EOF, 4},
			{"Javascript/success", bson.TypeJavaScript, strbytes, nil, 8},
			{"String/invalid length", bson.TypeString, strbytes[:2], io.EOF, 0},
			{"String/not enough bytes", bson.TypeString, strbytes[:5], io.EOF, 4},
			{"String/success", bson.TypeString, strbytes, nil, 8},
			{"Symbol/invalid length", bson.TypeSymbol, strbytes[:2], io.EOF, 0},
			{"Symbol/not enough bytes", bson.TypeSymbol, strbytes[:5], io.EOF, 4},
			{"Symbol/success", bson.TypeSymbol, strbytes, nil, 8},
			{"MaxKey/success", bson.TypeMaxKey, []byte{0x01}, nil, 0},
			{"MinKey/success", bson.TypeMinKey, []byte{0x01}, nil, 0},
			{"Null/success", bson.TypeNull, []byte{0x01}, nil, 0},
			{"Undefined/success", bson.TypeUndefined, []byte{0x01}, nil, 0},
			{
			...
			},
			{
				...
				bson.Type(0),
				nil,
				fmt.Errorf("attempted to skip unknown BSON type %v", bson.Type(0)), 0,
			},
		}
