...
func TestValueWriter(t *testing.T) {
	...
	t.Run("WriteBytes", func(t *testing.T) {
		t.Run("error on zero length", func(t *testing.T) {
			vw := newValueWriterFromSlice(nil)
			want := errors.New("cannot add bytes of zero length")
			got := vw.WriteBytes(nil)
			if !compareErrors(got, want) {
				t.Errorf("Did not received expected error. got %v; want %v", got, want)
			}
		})
		t.Run("cannot write length", func(t *testing.T) {
			vw := newValueWriterFromSlice(nil)
			vw.push(mCodeWithScope)
			vw.push(mElement)
			want := errMaxDocumentSizeExceeded{size: 65}
			maxSize = 64
			got := vw.WriteBytes(make([]byte, 61))
			if !compareErrors(got, want) {
				t.Errorf("Did not received expected error. got %v; want %v", got, want)
			}
			maxSize = math.MaxInt32
		...
		t.Run("success", func(t *testing.T) {
			...
			want, err := bson.NewDocument(bson.EC.String("hello", "world")).MarshalBSON()
			noerr(t, err)
			err = vw.WriteBytes(want)
			noerr(t, err)
			got := vw.buf
			if !bytes.Equal(got, want) {
		...
		t.Run("success code with scope", func(t *testing.T) {
			vw := newValueWriterFromSlice(make([]byte, 0, 512))
			_, err := vw.WriteDocument()
			noerr(t, err)
			_, err = vw.WriteDocumentElement("foo")
			noerr(t, err)
			_, err = vw.WriteCodeWithScope("var hello = 'world';")
			noerr(t, err)
			...
			if vw.stack[vw.frame].mode != mTopLevel {
				t.Errorf("Not in correct mode after writing bytes. got %v; want %v", vw.stack[vw.frame].mode, mTopLevel)
			}
			err = vw.WriteDocumentEnd()
			noerr(t, err)

			want, err := bson.NewDocument(bson.EC.CodeWithScope("foo", "var hello = 'world';", doc)).MarshalBSON()
			noerr(t, err)
