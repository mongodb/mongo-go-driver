...
func MarshalExtJSON(val interface{}, canonical bool) ([]byte, error) {
	return MarshalExtJSONWithRegistry(defaultRegistry, val, canonical)
}
...
// will be grown.
func MarshalExtJSONAppend(dst []byte, val interface{}, canonical bool) ([]byte, error) {
	return MarshalExtJSONAppendWithRegistry(defaultRegistry, dst, val, canonical)
}

// MarshalExtJSONWithRegistry returns the extended JSON encoding of val using Registry r.
func MarshalExtJSONWithRegistry(r *Registry, val interface{}, canonical bool) ([]byte, error) {
	dst := make([]byte, 0, defaultDstCap)
	return MarshalExtJSONAppendWithRegistry(r, dst, val, canonical)
}
...
func MarshalExtJSONAppendWithRegistry(r *Registry, dst []byte, val interface{}, canonical bool) ([]byte, error) {
	ejvw := ejvwPool.Get().(*extJSONValueWriter)
	defer ejvwPool.Put(ejvw)

	ejvw.reset(dst, canonical)
