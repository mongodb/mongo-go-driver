...
// ValueDecoderLegacy is the interface implemented by types that can handle the decoding of a value.
...
type ValueDecoderLegacy interface {
	DecodeValueLegacy(DecodeContext, bsonrw.ValueReader, interface{}) error
}

// ValueDecoderLegacyFunc is an adapter function that allows a function with the
// correct signature to be used as a ValueDecoder.
type ValueDecoderLegacyFunc func(DecodeContext, bsonrw.ValueReader, interface{}) error

// DecodeValueLegacy implements the ValueDecoder interface.
func (fn ValueDecoderLegacyFunc) DecodeValueLegacy(dc DecodeContext, vr bsonrw.ValueReader, val interface{}) error {
	return fn(dc, vr, val)
}

// func (fn ValueDecoderLegacyFunc) DecodeValue(dc DecodeContext, vr bsonrw.ValueReader, val reflect.Value) error {
// 	return fn(dc, vr, val.Interface())
// }
