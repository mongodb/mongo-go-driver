...
func (vr *valueReader) ReadBytes(dst []byte) ([]byte, error) {
	switch vr.stack[vr.frame].mode {
	case mTopLevel, mDocument, mCodeWithScope, mArray:
		length, err := vr.peakLength()
		if err != nil {
			return dst, err
		}
		defer vr.pop()
		return vr.appendBytes(dst, length)
	case mElement, mValue:
	default:
		return nil, vr.invalidTransitionErr(0)
	}
