...
// with the provided RegistryBuilder. if rb is nil, a new RegistryBuilder will be created, the newly
// created RegistryBuilder will contain the default encode and decode methods provided by
// bsoncodec.DefaultValueEncoders and bsoncodec.DefaultValueDecoders, respectively.
func (pc PrimitiveCodecs) RegisterPrimitiveCodecs(rb *bsoncodec.RegistryBuilder) *bsoncodec.RegistryBuilder {
	if rb == nil {
		rb = bsoncodec.NewEmptyRegistryBuilder()
		rb = bsoncodec.DefaultValueEncoders{}.RegisterDefaultEncoders(rb)
		rb = bsoncodec.DefaultValueDecoders{}.RegisterDefaultDecoders(rb)
	}
...
func (pv PrimitiveCodecs) ArrayEncodeValue(ec bsoncodec.EncodeContext, vw bsonrw.ValueWriter, i interface{}) error {
	arr, ok := i.(*Array)
	if !ok {
