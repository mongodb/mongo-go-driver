...
func (sc *StructCodec) DecodeValue(r DecodeContext, vr ValueReader, i interface{}) error {
	...
	for {
		...
		if !exists {
			...
			// This functionality seems appropriate for the struct codec. We additionally should
			// check for an inline map, and if there is one we can put these values in there.
			err = vr.Skip()
			if err != nil {
				return err
			}
		...
		if ec, ok := fd.codec.(*elementCodec); ok {
			err = ec.decodeValue(r, vr, name, field.Interface().(**Element))
			if err != nil {
				return err
			}
			continue
		...
		err = fd.codec.DecodeValue(r, vr, field.Interface())
		if err != nil {
			return err
		}
	}
