...
func (v Valuev2) MarshalBSONValue() (bsontype.Type, []byte, error) {
	...
	case TypeBinary:
		bin := v.Binary()
		data = bsoncore.AppendBinary(data, bin.Subtype, bin.Data)
	case TypeUndefined:
	case TypeObjectID:
	...
	case TypeRegex:
		regex := v.Regex()
		data = bsoncore.AppendRegex(data, regex.Pattern, regex.Options)
	case TypeDBPointer:
		dbptr := v.DBPointer()
		data = bsoncore.AppendDBPointer(data, dbptr.DB, dbptr.Pointer)
	case TypeJavaScript:
	...
	case TypeCodeWithScope:
		cws := v.CodeWithScope()
		var scope []byte
		scope, err = cws.Scope.MarshalBSON()
		if err != nil {
			break
		}
		data = bsoncore.AppendCodeWithScope(data, string(cws.Code), scope)
	case TypeInt32:
		data = bsoncore.AppendInt32(data, v.Int32())
	case TypeTimestamp:
		ts := v.Timestamp()
		data = bsoncore.AppendTimestamp(data, ts.T, ts.I)
	case TypeInt64:
...
func (v *Valuev2) UnmarshalBSONValue(t bsontype.Type, data []byte) error {
	...
	case TypeBinary:
		...
		*v = Binary(BinaryPrimitive{Subtype: subtype, Data: data})
	case TypeUndefined:
		*v = Undefined(UndefinedPrimitive{})
	case TypeObjectID:
	...
	case TypeDateTime:
		...
		*v = DateTime(DateTimePrimitive(dt))
	case TypeNull:
		*v = Null(NullPrimitive{})
	case TypeRegex:
		var pattern, options string
		pattern, options, rem, ok = bsoncore.ReadRegex(data)
		*v = Regex(RegexPrimitive{Pattern: pattern, Options: options})
	case TypeDBPointer:
		var ns string
		var ptr objectid.ObjectID
		ns, ptr, rem, ok = bsoncore.ReadDBPointer(data)
		*v = DBPointer(DBPointerPrimitive{DB: ns, Pointer: ptr})
	case TypeJavaScript:
		var js string
		js, rem, ok = bsoncore.ReadJavaScript(data)
		*v = JavaScript(JavaScriptCodePrimitive(js))
	case TypeSymbol:
		var symbol string
		symbol, rem, ok = bsoncore.ReadSymbol(data)
		*v = Symbol(SymbolPrimitive(symbol))
	case TypeCodeWithScope:
		var raw []byte
		var code string
		...
		*v = CodeWithScope(CodeWithScopePrimitive{Code: JavaScriptCodePrimitive(code), Scopev2: scope})
	case TypeInt32:
		var i32 int32
	...
	case TypeTimestamp:
		...
		*v = Timestamp(TimestampPrimitive{T: t, I: i})
	case TypeInt64:
		var i64 int64
	...
	case TypeDecimal128:
		...
		*v = Decimal128(d128)
	case TypeMinKey:
		*v = MinKey(MinKeyPrimitive{})
	case TypeMaxKey:
		*v = MaxKey(MaxKeyPrimitive{})
	default:
		err = fmt.Errorf("invalid BSON type %v", t)
	}
