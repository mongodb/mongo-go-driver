...
func (mc *MapCodec) DecodeValue(dc DecodeContext, vr ValueReader, i interface{}) error {
	...
	if val.Kind() != reflect.Map || val.Type().Key().Kind() != reflect.String || !val.CanSet() {
		return fmt.Errorf("%T can only encode settable (non-nil) maps with string keys", mc)
	}
	...
	for {
		...
		val.SetMapIndex(reflect.ValueOf(key), mvval)
	}
