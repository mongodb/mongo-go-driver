...
func TestProvidedCodecs(t *testing.T) {
			...
			[]deccase{
				{
					...
					&llValueReaderWriter{},
					llvrwNothing,
					fmt.Errorf("%T can only encode settable (non-nil) maps with string keys", &MapCodec{}),
				},
				{
					"wrong kind (non-string key)",
					map[int]interface{}{},
			...
			[]deccase{
				{
					...
					&llValueReaderWriter{},
					llvrwNothing,
					fmt.Errorf("%T can only decode arrays and slices", &SliceCodec{}),
				},
				{
					"can set false",
					(*[]string)(nil),
					...
					&llValueReaderWriter{},
					llvrwNothing,
					fmt.Errorf("%T cannot decode into a nil pointer", &SliceCodec{}),
				},
				{
					"ReadArray Error",
					[]interface{}{},
					nil,
					&llValueReaderWriter{err: errors.New("ra error"), errAfter: llvrwReadArray},
					llvrwReadArray,
					errors.New("ra error"),
				},
				{
				...
				},
				{
					...
					[]string{},
					&DecodeContext{Registry: NewRegistryBuilder().Build()},
					&llValueReaderWriter{err: errors.New("rv error"), errAfter: llvrwReadValue},
					llvrwReadValue,
					errors.New("rv error"),
				},
				{
	...
	t.Run("ArrayCodec", func(t *testing.T) {
		...
		t.Run("DecodeValue", func(t *testing.T) {
			...
			t.Run("ReadArray Error", func(t *testing.T) {
				...
				got := (&ArrayCodec{}).DecodeValue(DecodeContext{}, llvrw, NewArray())
				if !compareErrors(got, want) {
					t.Errorf("Errors do not match. got %v; want %v", got, want)
				}
			})
			t.Run("decode array errors", func(t *testing.T) {
				...
				for _, tc := range testCases {
					t.Run(tc.name, func(t *testing.T) {
						err := (&ArrayCodec{}).DecodeValue(tc.dc, tc.llvrw, NewArray())
						if !compareErrors(err, tc.err) {
							t.Errorf("Errors do not match. got %v; want %v", err, tc.err)
						}
					})
	...
	t.Run("SliceCodec/DecodeValue/can't set slice", func(t *testing.T) {
		var val []string
		want := fmt.Errorf("cannot set value of %T", val)
