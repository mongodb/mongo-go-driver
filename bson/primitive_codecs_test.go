	...
	*pdatetime = primitive.DateTime(1234567890)
	...
	t.Run("ArrayEncodeValue", func(t *testing.T) {
		...
		t.Run("WriteArray Error", func(t *testing.T) {
			...
			got := (PrimitiveCodecs{}).ArrayEncodeValue(bsoncodec.EncodeContext{}, llvrw, make(bsonx.Arr, 0))
			if !compareErrors(got, want) {
				t.Errorf("Errors do not match. got %v; want %v", got, want)
			}
		})
		...
		testCases := []struct {
			...
			},
			{
				...
				docToBytes(bsonx.Doc{
					...
					{"f", Array(bsonx.Arr{Double(3.14159)})},
					...
					{"aa", Array(bsonx.Arr{Int64(5), Double(10.10)})},
					...
					{"ag", Array(bsonx.Arr{Document(bsonx.Doc{{"pi", Double(3.14159)}}), Null()})},
				}),
				nil,
			},
		}
	...
	*psymbol = primitive.Symbol("foobarbaz")
	...
	t.Run("ArrayDecodeValue", func(t *testing.T) {
		...
		t.Run("ReadArray Error", func(t *testing.T) {
			...
			got := pc.ArrayDecodeValue(bsoncodec.DecodeContext{}, llvrw, new(bsonx.Arr))
			if !compareErrors(got, want) {
				t.Errorf("Errors do not match. got %v; want %v", got, want)
			}
		})
		t.Run("decode array errors", func(t *testing.T) {
			...
			for _, tc := range testCases {
				t.Run(tc.name, func(t *testing.T) {
					err := pc.ArrayDecodeValue(tc.dc, tc.llvrw, new(bsonx.Arr))
					if !compareErrors(err, tc.err) {
						t.Errorf("Errors do not match. got %v; want %v", err, tc.err)
					}
				})
		...
		*str = "bar"
		...
		testCases := []struct {
			...
			},
			{
				...
				docToBytes(bsonx.Doc{{"Z", Array(bsonx.Arr{Int64(5), Double(10.1)})}}),
				nil,
			},
			{
				"map[string][]*url.URL",
