	...
	t.Run("DocumentEncodeValue", func(t *testing.T) {
		...
		t.Run("encodeDocument errors", func(t *testing.T) {
			...
			testCases := []struct {
				...
				{
					"WriteDocument (Lookup)", bsoncodec.EncodeContext{Registry: bsoncodec.NewEmptyRegistryBuilder().Build()},
				...
				{
					"WriteArray (Lookup)", bsoncodec.EncodeContext{Registry: bsoncodec.NewEmptyRegistryBuilder().Build()},
					&bsonrwtest.ValueReaderWriter{T: t},
				...
				{
					"WriteCodeWithScope (Lookup)", bsoncodec.EncodeContext{Registry: bsoncodec.NewEmptyRegistryBuilder().Build()},
				...
				{
					"Invalid Type", ec,
					&bsonrwtest.ValueReaderWriter{T: t, BSONType: Type(0)},
					NewDocument(badelem),
		...
		*str = "bar"
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				noerr(t, err)
				enc, err := bsoncodec.NewEncoder(DefaultRegistry, vw)
				noerr(t, err)
	...
	t.Run("DocumentDecodeValue", func(t *testing.T) {
		...
		t.Run("decodeDocument errors", func(t *testing.T) {
			...
			testCases := []struct {
				...
				{
					"ReadDocument (Lookup)", bsoncodec.DecodeContext{Registry: bsoncodec.NewEmptyRegistryBuilder().Build()},
					&bsonrwtest.ValueReaderWriter{T: t, BSONType: bsontype.EmbeddedDocument},
					bsoncodec.ErrNoDecoder{Type: tDocument},
				},
				{
					"ReadArray (Lookup)", bsoncodec.DecodeContext{Registry: bsoncodec.NewEmptyRegistryBuilder().Build()},
				...
				{
					"ReadCodeWithScope (Lookup)", bsoncodec.DecodeContext{Registry: bsoncodec.NewEmptyRegistryBuilder().Build()},
		...
		t.Run("success", func(t *testing.T) {
			...
			noerr(t, err)
			err = pc.DocumentDecodeValue(dc, bsonrw.NewValueReader(b), &got)
			noerr(t, err)
			if !got.Equal(want) {
				t.Error("Documents do not match")
				t.Errorf("\ngot :%v\nwant:%v", got, want)
		...
		t.Run("Decode", func(t *testing.T) {
			for _, tc := range testCases {
				t.Run(tc.name, func(t *testing.T) {
					...
					dec, err := bsoncodec.NewDecoder(DefaultRegistry, vr)
					noerr(t, err)
			...
			for _, tc := range testCases {
				t.Run(tc.name, func(t *testing.T) {
					...
					t.Run("Lookup failure", func(t *testing.T) {
						val := new(interface{})
						dc := bsoncodec.DecodeContext{Registry: bsoncodec.NewEmptyRegistryBuilder().Build()}
					...
					t.Run("DecodeValue failure", func(t *testing.T) {
						...
						dc := bsoncodec.DecodeContext{
							Registry: bsoncodec.NewEmptyRegistryBuilder().RegisterDecoder(reflect.TypeOf(tc.val), llc).Build(),
						}
