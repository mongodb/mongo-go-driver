...
func newDocumentValueReader(d *Document) *documentValueReader {
...
func (vr *documentValueReader) invalidTransitionErr(destination mode) error {
	te := transitionError{
...
func (dvr *documentValueReader) ensureElementValue(t Type, destination mode) error {
	switch dvr.stack[dvr.frame].mode {
	case mElement, mValue:
...
func (dvr *documentValueReader) ReadArray() (ArrayReader, error) {
	if err := dvr.ensureElementValue(TypeArray, mArray); err != nil {
		return nil, err
	}
