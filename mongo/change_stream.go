...
func newChangeStream(ctx context.Context, config changeStreamConfig, pipeline interface{},
	...
	if resumeToken != nil {
		marshaledToken, cs.err = bson.Marshal(resumeToken)
		if cs.err != nil {
			return nil, cs.Err()
		}
	}
	...
	cs.err = cs.executeOperation(ctx, false)
	if cs.err != nil {
		return nil, cs.Err()
	}
...
func (cs *ChangeStream) executeOperation(ctx context.Context, resuming bool) error {
	...
	if resuming {
		...
		server, cs.err = cs.client.topology.SelectServer(ctx, cs.selector)
		conn, cs.err = server.Connection(ctx)
		...
		pipDoc, cs.err = bsoncore.AppendDocumentEnd(pipDoc, pipIdx)
		if cs.err != nil {
			closeImplicitSession(cs.sess)
			return cs.Err()
		}
