...
func (cs *ChangeStream) executeOperation(ctx context.Context, resuming bool) error {
	...
	if resuming {
		...
		if plArr, cs.err = cs.buildPipelineArray(); cs.err != nil {
			return cs.Err()
		}
	...
	if cs.updatePbrtFromCommand(); cs.options.StartAtOperationTime == nil && cs.options.ResumeAfter == nil &&
		cs.options.StartAfter == nil && conn.Description().WireVersion.Max >= 7 && cs.emptyBatch() && cs.resumeToken == nil {
...
func (cs *ChangeStream) Close(ctx context.Context) error {
	...
	err := replaceErrors(cs.cursor.Close(ctx))
	cs.cursor = nil
	return err
}
