...
func (cs *ChangeStream) executeOperation(ctx context.Context, resuming bool) error {
	...
	if original := cs.aggregate.Execute(ctx); original != nil {
		retryableRead := cs.client != nil && cs.client.retryReads && (cs.client.topology.RetryType() == driver.RetryRead || cs.client.topology.RetryType() == driver.RetryWrite) && description.SessionsSupported(conn.Description().WireVersion)
		if !retryableRead {
			...
			if tt.Retryable() {
				...
				if server, err = cs.client.topology.SelectServer(ctx, cs.selector); err == nil {
					if conn, err = server.Connection(ctx); err == nil {
						if (cs.client.topology.RetryType() == driver.RetryRead || cs.client.topology.RetryType() == driver.RetryWrite) && description.SessionsSupported(conn.Description().WireVersion) {
							cs.aggregate.Deployment(driver.SingleConnectionDeployment{
								C: conn,
							})
