...
func (cs *ChangeStream) storeResumeToken() error {
	br, err := cs.cursor.DecodeBytes(nil)
	if err != nil {
		return err
	}

	idVal, err := bson.Raw(br).LookupErr("_id")
	if err != nil {
...
func (cs *ChangeStream) DecodeBytes(dst []byte) (bson.Raw, error) {
	if cs.cursor == nil {
		return dst, ErrNilCursor
	}
	if cs.err != nil {
		return dst, cs.err
	}

	return cs.cursor.DecodeBytes(dst)
}
