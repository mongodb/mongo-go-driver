...
func TestGridFSRetryableReadSpec(t *testing.T) {
	...
	copts := options.Client().ApplyURI(cs.String()).SetMonitor(gridFSMonitor)
	retryOnClient, err := mongo.NewClient(copts)
	...
	var f bool
	copts.RetryReads = &f
	retryOffClient, err := mongo.NewClient(copts)
	require.NoError(t, err, "unable to create client")
	err = retryOffClient.Connect(ctx)
	require.NoError(t, err, "unable to connect to client")
	for _, testFile := range testhelpers.FindJSONFilesInDir(t, gridFSRetryDir) {
		runGridFSRetryReadTestFile(t, testFile, retryOnClient, retryOffClient)
	}
	_ = retryOnClient.Disconnect(ctx)
	_ = retryOffClient.Disconnect(ctx)
}

func runGridFSRetryReadTestFile(t *testing.T, testFileName string, retryOnClient, retryOffClient *mongo.Client) {
