...
func compareValues(expected *bson.Value, actual *bson.Value) bool {
	if expected.IsNumber() {
		...
		expectedNum := expected.Double() // from JSON file
		switch actual.Type() {
		case bson.TypeInt32:
			return actual.Int32() == int32(expectedNum)
		case bson.TypeInt64:
			return actual.Int64() == int64(expectedNum)
		case bson.TypeDouble:
			return actual.Double() == expectedNum
		}
...
func runUploadTest(t *testing.T, test test, bucket *Bucket) {
	...
	argsBytes, err := test.Act.Arguments.MarshalJSON()
	testhelpers.RequireNil(t, err, "error marshalling arguments: %s", err)

	args, err := bson.ParseExtJSONObject(string(argsBytes))
	testhelpers.RequireNil(t, err, "error converting arguments to BSON doc: %s", err)
	...
	for _, assertData := range assert.Data {
		...
		// after writing the files collection document on Close(), the stream has the _id of the files coll doc stored
		// in the FileID field. Each chunk's files_id should be equal to this ID
		compareFiles(t)
		compareChunks(t, stream.FileID)
	}
}

func runArrangeDelete(t *testing.T, deletes *bson.Array, coll *mongo.Collection) {
...
func runArrangeUpdate(t *testing.T, updates *bson.Array, coll *mongo.Collection) {
...
func runDownloadTest(t *testing.T, test test, bucket *Bucket) {
	...
	for _, msg := range test.Arrange.Data {
		...
		if _, err = msgDoc.LookupErr("delete"); err == nil {
			...
			runArrangeDelete(t, msgDoc.Lookup("deletes").MutableArray(), chunks)
		} else if _, err = msgDoc.LookupErr("update"); err == nil {
			runArrangeUpdate(t, msgDoc.Lookup("updates").MutableArray(), chunks)
		}
	}
...
func runDownloadByNameTest(t *testing.T, test test, bucket *Bucket) {
	// act section
	argsBytes, err := test.Act.Arguments.MarshalJSON()
	testhelpers.RequireNil(t, err, "error marshalling act: %s", err)

	args, err := bson.ParseExtJSONObject(string(argsBytes))
	testhelpers.RequireNil(t, err, "error creating act doc: %s", err)
