...
func (b *Bucket) openDownloadStream(filter interface{}, opts ...*options.FindOptions) (*DownloadStream, error) {
	...
	fileRdr, err := cursor.DecodeBytes(nil)
	if err != nil {
		return nil, err
	}

	fileLenElem, err := bson.Raw(fileRdr).LookupErr("length")
	if err != nil {
		return nil, err
	}
	fileIDElem, err := bson.Raw(fileRdr).LookupErr("_id")
	if err != nil {
		return nil, err
	}
...
func createIndexIfNotExists(ctx context.Context, iv mongo.IndexView, model mongo.IndexModel) error {
	...
	var found bool
	for c.Next(ctx) {
		rdr, err := c.DecodeBytes(nil)
		if err != nil {
			return err
		}

		keyElem, err := bson.Raw(rdr).LookupErr("key")
		if err != nil {
			return err
		}
