...
func NewBucket(db *mongo.Database, opts ...BucketOptioner) (*Bucket, error) {
	...
	b.readBuf = make([]byte, 0, b.chunkSize)
	b.writeBuf = make([]byte, 0, b.chunkSize)
...
// The write deadline for this bucket must be set before creating an upload stream.
...
func (b *Bucket) OpenUploadStreamWithID(fileID objectid.ObjectID, filename string, opts ...UploadOptioner) (*UploadStream, error) {
	ctx, cancel := context.WithDeadline(context.Background(), b.writeDeadline)
	defer cancel()
...
// The write deadline for this bucket must be set before uploading from a stream.
...
// The read deadline for this bucket must be set before creating a download stream.
...
// The read deadline for this bucket must be set before downloading to a stream.
func (b *Bucket) DownloadToStream(fileID objectid.ObjectID, stream io.Writer) error {
...
func (b *Bucket) DownloadToStreamByName(filename string, stream io.Writer, opts ...NameOptioner) error {
...
// The write deadline for this bucket must be set before deleting a file.
func (b *Bucket) Delete(fileID objectid.ObjectID) error {
	...
	_, err := b.filesColl.DeleteOne(ctx, bson.NewDocument(
		bson.EC.ObjectID("_id", fileID),
	))
	if err != nil {
		_ = b.deleteChunks(ctx, fileID)
		return err
	}
...
// The read deadline for this bucket must be set before running a find operation.
func (b *Bucket) Find(filter interface{}, opts ...FindOptioner) (mongo.Cursor, error) {
	ctx, cancel := context.WithDeadline(context.Background(), b.readDeadline)
	defer cancel()
...
func (b *Bucket) downloadToStream(ds *DownloadStream, stream io.Writer) error {
