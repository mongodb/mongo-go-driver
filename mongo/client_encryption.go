...
func NewClientEncryption(ctx context.Context, opts ...*options.ClientEncryptionOptions) (*ClientEncryption, error) {
	ce := &ClientEncryption{}
	ceo := options.MergeClientEncryptionOptions(opts...)

	// create keyVaultClient
	var err error
	ce.keyVaultClient, err = Connect(ctx, ceo.KeyVaultClientOptions)
	if err != nil {
		return nil, err
	}
...
func (ce *ClientEncryption) Encrypt(ctx context.Context, val bson.RawValue, opts ...*options.EncryptOptions) (primitive.Binary, error) {
	...
	subtype, data, err := ce.crypt.ExplicitlyEncrypt(ctx, bsoncore.Value{Type: val.Type, Data: val.Value}, transformed)
	if err != nil {
		return primitive.Binary{}, err
	}
	return primitive.Binary{Subtype: subtype, Data: data}, nil
...
func (ce *ClientEncryption) Decrypt(ctx context.Context, val primitive.Binary) (bson.RawValue, error) {
	decrypted, err := ce.crypt.ExplicitlyDecrypt(ctx, val.Subtype, val.Data)
	if err != nil {
