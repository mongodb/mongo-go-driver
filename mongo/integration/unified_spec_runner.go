...
// RunSpecTestDirectory takes in the location of a directory containing only tests in the unified test format. It
// expects that mtest.Setup is run before and mtest.Teardown is run afterward.
// dataPath: path to the test directory
// specDir: name of directory for a spec folder
func RunSpecTestDirectory(t *testing.T, dataPath, specDir string) {
	for _, fileName := range jsonFilesInDir(t, path.Join(dataPath, specDir)) {
		t.Run(fileName, func(t *testing.T) {
			RunSpecTestFile(t, dataPath, specDir, fileName)
		})
	}
}

// RunSpecTestFile takes in the location of a file in the unified spec format and runs the tests in it. It expects that
// the mtest.Setup is run before and mtest.Teardown is run afterward.
// dataPath: path to the test directory
// specDir: name of directory for a spec folder
// fileName: name of test file in specDir
func RunSpecTestFile(t *testing.T, dataPath, specDir, fileName string) {
