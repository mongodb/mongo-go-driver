...
func (t *T) RunOpts(name string, opts *Options, callback func(*T)) {
	t.T.Run(name, func(wrapped *testing.T) {
		...
		// defer dropping all collections only if the sub-test is not running against a mock
		if sub.clientType != Mock && sub.Client != nil {
			defer func() {
		...
		// for shareClient, take the client from the parent
