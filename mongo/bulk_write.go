...
func (bw *bulkWrite) runDelete(ctx context.Context, batch bulkWriteBatch) (operation.DeleteResult, error) {
	...
	for _, model := range batch.models {
		...
		if dom, ok := model.(*DeleteOneModel); ok {
			doc, err = createDeleteDoc(dom.Filter, dom.Collation, true, bw.collection.registry)
		} else if dmm, ok := model.(*DeleteManyModel); ok {
			doc, err = createDeleteDoc(dmm.Filter, dmm.Collation, false, bw.collection.registry)
		}
...
func (bw *bulkWrite) runUpdate(ctx context.Context, batch bulkWriteBatch) (operation.UpdateResult, error) {
	...
	for i, model := range batch.models {
		...
		if rom, ok := model.(*ReplaceOneModel); ok {
			doc, err = createUpdateDoc(rom.Filter, rom.Replacement, nil, rom.Collation, rom.Upsert, false,
				bw.collection.registry)
		} else if uom, ok := model.(*UpdateOneModel); ok {
			doc, err = createUpdateDoc(uom.Filter, uom.Update, uom.ArrayFilters, uom.Collation, uom.Upsert, false,
				bw.collection.registry)
		} else if umm, ok := model.(*UpdateManyModel); ok {
			doc, err = createUpdateDoc(umm.Filter, umm.Update, umm.ArrayFilters, umm.Collation, umm.Upsert, true,
				bw.collection.registry)
		}
		if err != nil {
...
func createBatches(models []WriteModel, ordered bool) []bulkWriteBatch {
	...
	for _, model := range models {
		...
		case *DeleteOneModel:
			if deleteOneInd == -1 {
				deleteOneInd = numBatches
				numBatches++
			}

			batches[deleteOneInd].models = append(batches[deleteOneInd].models, model)
		case *DeleteManyModel:
			if deleteManyInd == -1 {
				deleteManyInd = numBatches
				numBatches++
				batches[deleteManyInd].canRetry = false
			}

			batches[deleteManyInd].models = append(batches[deleteManyInd].models, model)
		case *ReplaceOneModel, *UpdateOneModel:
			if updateOneInd == -1 {
				updateOneInd = numBatches
				numBatches++
			}

			batches[updateOneInd].models = append(batches[updateOneInd].models, model)
		case *UpdateManyModel:
			if updateManyInd == -1 {
				updateManyInd = numBatches
				numBatches++
				batches[updateManyInd].canRetry = false
			}

			batches[updateManyInd].models = append(batches[updateManyInd].models, model)
		}
	}
