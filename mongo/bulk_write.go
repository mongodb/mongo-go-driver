...
func createBatches(models []WriteModel, ordered bool) []bulkWriteBatch {
	...
	opIndexes := make(map[writeCommandKind]int)

	// TODO: GODRIVER-1157 fix batching once operation retryability is fixed
	for _, model := range models {
		switch model.(type) {
		case *InsertOneModel:
			if _, found := opIndexes[insertCommand]; !found {
				// this is the first InsertOneModel
				opIndexes[insertCommand] = numBatches
				numBatches++
			}
			ind := opIndexes[insertCommand]
			batches[ind].models = append(batches[ind].models, model)
		case *DeleteOneModel:
			if _, found := opIndexes[deleteOneCommand]; !found {
				opIndexes[deleteOneCommand] = numBatches
				numBatches++
			}
			ind := opIndexes[deleteOneCommand]
			batches[ind].models = append(batches[ind].models, model)
		case *DeleteManyModel:
			if _, found := opIndexes[deleteManyCommand]; !found {
				opIndexes[deleteManyCommand] = numBatches
				numBatches++
				batches[opIndexes[deleteManyCommand]].canRetry = false
			}
			ind := opIndexes[deleteManyCommand]
			batches[ind].models = append(batches[ind].models, model)
		case *ReplaceOneModel, *UpdateOneModel:
			if _, found := opIndexes[updateOneCommand]; !found {
				opIndexes[updateOneCommand] = numBatches
				numBatches++
			}
			ind := opIndexes[updateOneCommand]
			batches[ind].models = append(batches[ind].models, model)
		case *UpdateManyModel:
			if _, found := opIndexes[updateManyCommand]; !found {
				opIndexes[updateManyCommand] = numBatches
				numBatches++
				batches[opIndexes[updateManyCommand]].canRetry = false
			}
			ind := opIndexes[updateManyCommand]
			batches[ind].models = append(batches[ind].models, model)
		}
	}
