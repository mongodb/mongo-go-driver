...
func runCRUDTestFileV2(t *testing.T, filepath string) {
	...
	db = createMonitoredTestDatabase(t, &dbName)
...
func testOperations(t *testing.T, coll *Collection, db *Database, test *testCaseV2) {
	...
	for _, operation := range test.Operations {
		...
		default:
			t.Fatalf("Not recognized or setup yet test: %v", operation.Name)
		}
	}
	...
	if test.Expectations != nil && len(test.Expectations) != 0 {
		...
		if test.Outcome.Collection != nil {
			...
			verifyCollectionContents2(t, outColl, test.Outcome.Collection.Data)
		}
...
func runOperationAggregate(t *testing.T, coll *Collection, db *Database, oper *op, test *testCaseV2) {
	if oper.Object == "database" && os.Getenv("TOPOLOGY") == "sharded_cluster" {
	...
	if oper.Object == "collection" {
		if coll == nil {
		...
		} else if oper.CollectionOptions != nil && oper.CollectionOptions.ReadConcern != nil {
			...
			if err != nil {
				t.Fatalf("unable to set readconcern")
			}
		}
		a = coll
	}
	...
	if !out && oper.Result != nil {
		errored = verifyCursorResultWithErrors(t, cursor, oper.Result, oper.Error, errored)
	}

	if oper.Error {
		require.True(t, errored, "failed to error when expected")
	}

}

func verifyCollectionContents2(t *testing.T, coll *Collection, result json.RawMessage) bool {
	cursor, err := coll.Find(context.Background(), bsonx.Doc{})
	require.NoError(t, err, "coll.Find failed")
	return verifyCursorResultWithErrors(t, cursor, result, false, false)
}

func verifyCursorResultWithErrors(t *testing.T, cur *Cursor, result json.RawMessage, causesError, errored bool) bool {
