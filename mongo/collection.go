...
func (coll *Collection) BulkWrite(ctx context.Context, models []WriteModel,
	...
	if sess == nil && coll.client.sessionPool != nil {
		var err error
		sess, err = session.NewClientSessionDeferImplicit(coll.client.sessionPool, coll.client.id, session.Implicit)
		if err != nil {
			return nil, err
		}
...
func (coll *Collection) insert(ctx context.Context, documents []interface{},
	...
	op := operation.NewInsert(docs...).
		Session(sess).WriteConcern(coll.writeConcern).CommandMonitor(coll.client.monitor).
