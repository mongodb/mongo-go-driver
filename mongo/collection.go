...
func (coll *Collection) updateOrReplaceOne(ctx context.Context, filter interface{},
	...
	updateDocs := []*bson.Document{
		bson.NewDocument(3).
			Append(bson.C.SubDocument("q", f), bson.C.SubDocument("u", u), bson.C.Boolean("multi", false)),
	}
...
func (coll *Collection) UpdateManyContext(ctx context.Context, filter interface{}, update interface{},
	...
	updateDocs := []*bson.Document{
		bson.NewDocument(3).
			Append(bson.C.SubDocument("q", f), bson.C.SubDocument("u", u), bson.C.Boolean("multi", true)),
	}
	// updateDocs := []oldbson.D{{
	// 	{Name: "q", Value: filter},
	// 	{Name: "u", Value: update},
...
func (coll *Collection) FindOneContext(ctx context.Context, filter interface{}, result interface{},
	...
	cursor, err := coll.FindContext(ctx, f, options...)
	if err != nil {
		return false, err
	}
