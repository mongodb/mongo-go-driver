...
func getStreamOptions(test *csTest) []changestreamopt.ChangeStream {
	if len(test.Options) > 0 {
		fmt.Printf("NON ZERO OPTIONS LENGTH")
	}
...
func compareCommands(t *testing.T, expected *bson.Document, actual *bson.Document) {
...
func compareCsStartedEvent(t *testing.T, expected json.RawMessage) {
	...
	expectedDoc, err := bson.ParseExtJSONObject(string(expected))
	testhelpers.RequireNil(t, err, "error transforming expected event: %s", err)
	...
	expectedCmd := expectedDoc.Lookup("command").MutableDocument()
...
func runCsTestFile(t *testing.T, globalClient *Client, path string) {
	...
	for _, test := range testfile.Tests {
		t.Run(test.Description, func(t *testing.T) {
			...
			clientColl := clientDb.Collection(testfile.CollectionName, collectionopt.WriteConcern(wcMajority))
			...
			if numChanges < len(test.Result.Success) {
				t.Fatalf("expected at least %d changes, got %d", len(test.Result.Success), numChanges)
			}
...
func insertOne(t *testing.T, coll *Collection, args map[string]interface{}) error {
	doc, err := TransformDocument(args["document"])
