			...
			t.Run("WithoutPBRTSupport", func(t *testing.T) {
				...
				for _, tc := range cases {
					t.Run(tc.name, func(t *testing.T) {
						...
						// iterate once and assert resume token matches _id of last document
						if !stream.Next(ctx) {
							t.Fatal("Next returned false, expected true")
						}

						var currDoc bson.Raw
						err = stream.Decode(&currDoc)
						testhelpers.RequireNil(t, err, "error decoding event: %v", err)
						compareResumeTokens(t, stream, currDoc.Lookup("_id").Document())
					})
				}
			})
		})
