...
func TestChangeStream_firstStage(t *testing.T) {
	...
	_, err := coll.InsertOne(context.Background(), bsonx.Doc{{"x", bson.Int32(1)}})
	require.NoError(t, err)
...
func TestChangeStream_trackResumeToken(t *testing.T) {
	...
	// Ensure the database is created.
	_, err := coll.InsertOne(context.Background(), bsonx.Doc{{"y", bson.Int32(1)}})
	require.NoError(t, err)
	...
	for i := 1; i <= 4; i++ {
		_, err = coll.InsertOne(context.Background(), bsonx.Doc{{"x", bson.Int32(int32(i))}})
		require.NoError(t, err)
	}
...
func TestChangeStream_errorMissingResponseToken(t *testing.T) {
	...
	changes, err := coll.Watch(context.Background(), []bsonx.Doc{
		{{"$project", bsonx.Document(bsonx.Doc{{"_id", bson.Int32(0)}})}},
	})
	require.NoError(t, err)
