...
func TestCollection_CountDocuments_withLimitOptions(t *testing.T) {
	...
	count, err := coll.CountDocuments(context.Background(), nil, options.Count().SetLimit(int64(3)))
	require.Nil(t, err)
	require.Equal(t, count, int64(3))
}
...
func TestCollection_CountDocuments_withSkipOptions(t *testing.T) {
	...
	count, err := coll.CountDocuments(context.Background(), nil, options.Count().SetSkip(int64(3)))
	require.Nil(t, err)
...
func TestCollection_EstimatedDocumentCount_withOption(t *testing.T) {
	...
	count, err := coll.EstimatedDocumentCount(context.Background(), options.EstimatedDocumentCount().SetMaxTimeMS(int32(100)))
	require.Nil(t, err)
	require.Equal(t, count, int64(5))
}
