...
func countDocumentsAggregatePipeline(registry *bsoncodec.Registry, filter interface{}, opts ...*options.CountOptions) (*bson.Array, error) {
	...
	if opts != nil {
		if opts[0].Skip != nil {
			pipeline.Append(bson.VC.Document(bson.NewDocument(bson.EC.Int64("$skip", *opts[0].Skip))))
		}
		if opts[0].Limit != nil {
			pipeline.Append(bson.VC.Document(bson.NewDocument(bson.EC.Int64("$limit", *opts[0].Limit))))
		}
	}
