...
func getSessionUUID(t *testing.T, cmd *bson.Document) []byte {
	...
	testhelpers.RequireNil(t, err, "key lsid not found in started command")
...
func TestSessions(t *testing.T) {
	...
	t.Run("TestClusterTime", func(t *testing.T) {
		// Test to see if $clusterTime is inlcuded in commands
		...
		for _, tc := range funcMap {
			t.Run(tc.name, func(t *testing.T) {
				...
				if tc.name == "InsertOne" {
					tc.opts[len(tc.opts)-1] = bson.NewDocument(bson.EC.Int32("InsertOneNewDoc", 1))
				} else if tc.name == "InsertMany" {
					tc.opts[len(tc.opts)-1] = []interface{}{bson.NewDocument(bson.EC.Int32("InsertManyNewDoc", 2))}
				}
	...
	t.Run("TestSessionArgsForClient", func(t *testing.T) {
		...
		client, db, _, funcMap := createFuncMap(t, "sessionArgsDb", "sessionArgsColl", false)
		err := db.Drop(ctx)
		testhelpers.RequireNil(t, err, "error dropping database: %s", err)
	...
	t.Run("TestEndSession", func(t *testing.T) {
		...
		client, db, _, funcMap := createFuncMap(t, "endSessionsDb", "endSessionsDb", false)
		err := db.Drop(ctx)
		testhelpers.RequireNil(t, err, "error dropping database: %s", err)
	...
	t.Run("TestImplicitSessionReturnedFromGetMore", func(t *testing.T) {
		...
		client := createTestClient(t)
		defer verifySessionsReturned(t, client) // will verify cursor returned session
	...
	t.Run("TestFindAndGetMoreSessionIDs", func(t *testing.T) {
		// test to make sure that GetMore commands have the same session ID as the associated Find command

		skipInvalidTopology(t)
		skipIfBelow36(t)

		primary := readpref.Primary()
		primaryPref := readpref.PrimaryPreferred()
		secondary := readpref.Secondary()
		secondaryPref := readpref.SecondaryPreferred()
		nearest := readpref.Nearest()

		topos := []string{"replica_set", "sharded_cluster"}
		prefs := []*readpref.ReadPref{primary, primaryPref, secondary, secondaryPref, nearest}

		client := createSessionsMonitoredClient(t)

		db := client.Database("TestFindGetMoreIDsDB")
		err := db.Drop(ctx)
		testhelpers.RequireNil(t, err, "error dropping database: %s", err)
		coll := db.Collection("TestFindGetMoreIDsColl")
		// add Majority write concern so reads from secondaries pass
		coll.writeConcern = writeconcern.New(writeconcern.WMajority())

		currTopo := os.Getenv("TOPOLOGY")
		for _, top := range topos {
			...
			for _, pref := range prefs {
				testName := fmt.Sprintf("Test%s%d", currTopo, pref.Mode())
				t.Run(testName, func(t *testing.T) {
					...
					coll.readPreference = pref
					cur, err := coll.Find(ctx, emptyDoc, findopt.BatchSize(2))
					require.Nil(t, err, "Error on find")

					testhelpers.RequireNotNil(t, sessionStarted, "started command for find was nil")
					findUUID := getSessionUUID(t, sessionStarted.Command)
