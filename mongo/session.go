...
func (s *sessionImpl) WithTransaction(ctx context.Context, fn func(sessCtx SessionContext) (interface{}, error), opts ...*options.TransactionOptions) (interface{}, error) {
	...
	CommitLoop:
		for {
			...
			if cerr, ok := err.(CommandError); ok {
				if cerr.HasErrorLabel(driver.UnknownTransactionCommitResult) && !IsMaxTimeMSExpiredError(cerr) {
					continue
				}
				...
				if IsMaxTimeMSExpiredError(cerr) || IsUnsatisfiableWriteConcernError(cerr) || IsUnknownReplWriteConcernError(cerr) {
					break CommitLoop
				}
			}
...
func (s *sessionImpl) StartTransaction(opts ...*options.TransactionOptions) error {
	...
	coreOpts := &session.TransactionOptions{
		...
		MaxCommitTimeMS: topts.MaxCommitTimeMS,
	}
...
func (s *sessionImpl) CommitTransaction(ctx context.Context) error {
	...
	if s.clientSession.CurrentMctms != nil {
		op.MaxTimeMS(int64(*s.clientSession.CurrentMctms / time.Millisecond))
	}
