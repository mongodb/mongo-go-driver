...
func newDatabase(client *Client, name string, opts ...*options.DatabaseOptions) *Database {
	dbOpt := options.ToDatabaseOptions(opts...)
...
func (db *Database) RunCommand(ctx context.Context, runCommand interface{}, opts ...*options.RunCmdOptions) (bson.Reader, error) {
	...
	runCmd := options.ToRunCmdOptions(opts...)
