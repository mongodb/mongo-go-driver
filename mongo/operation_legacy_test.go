...
type opQuery struct {
	...
	query, returnFieldsSelector bsoncore.Document
}
...
func validateQueryWiremessage(t *testing.T, wm []byte, expected opQuery) {
	...
	if !bytes.Equal(query, expected.query) {
		t.Fatalf("query mismatch; expected %v, got %v", expected.query, query)
	}
	...
	if !bytes.Equal(returnFieldsSelector, expected.returnFieldsSelector) {
		t.Fatalf("returnFieldSelector mismatch; expected %v, got %v", expected.returnFieldsSelector,
...
func TestOperationLegacy(t *testing.T) {
	...
	t.Run("VerifyWiremessage", func(t *testing.T) {
		...
		findQueryDocBytes, _ := bson.Marshal(findQueryDoc)
		findProjectionBytes, _ := bson.Marshal(projection)
		findQuery := opQuery{
		...
		listIndexesBytes, _ := bson.Marshal(bson.D{
