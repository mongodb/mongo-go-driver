...
func TestConnection(t *testing.T) {
	isMaster, err := (&connection.CommandIsMaster{}).Encode()
	if err != nil {
	...
	t.Run("Expired", func(t *testing.T) {
		t.Run("Idle Timeout", func(t *testing.T) {
			c := testConnection(t, connection.WithIdleTimeout(func(time.Duration) time.Duration { return 5 * time.Millisecond }))
			defer c.Close()
			if c.Expired() != false {
				t.Errorf("Newly created connection should not be expired.")
			}
		...
		t.Run("Lifetime Timeout", func(t *testing.T) {
			c := testConnection(t, connection.WithLifeTimeout(func(time.Duration) time.Duration { return 5 * time.Millisecond }))
			defer c.Close()
			if c.Expired() != false {
				t.Errorf("Newly created connection should not be expired.")
			}
...
func testConnection(t *testing.T, opts ...connection.Option) connection.Connection {
	opts = append(opts, connection.WithAppName(func(string) string { return "mongo-go-driver-test" }))
	...
	c, err := connection.New(context.Background(), addr.Addr(*host), opts...)
	if err != nil {
