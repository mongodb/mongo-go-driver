...
func selectForReplicaSet(rp *readpref.ReadPref, t description.Topology, candidates []description.Server) ([]description.Server, error) {
	...
	case readpref.PrimaryPreferredMode:
		...
		if len(selected) == 0 {
			...
			return selectByTagSet(selected, description.ConvertTagSets(rp.TagSets())), nil
		}

		return selected, nil
	case readpref.SecondaryPreferredMode:
		selected := selectSecondaries(rp, candidates)
		selected = selectByTagSet(selected, description.ConvertTagSets(rp.TagSets()))
		if len(selected) > 0 {
			return selected, nil
		}
		return selectByKind(candidates, description.RSPrimary), nil
...
func selectByTagSet(candidates []description.Server, tagSets []description.TagSet) []description.Server {
	if len(tagSets) == 0 {
		return candidates
	}
