...
// closed connection.
var ErrSubscribeAfterClosed = errors.New("cannot subscribe to a server after closing it")
...
// New creates a new server. The mongodb server at the address will be monitored
...
func (s *Server) heartbeat(conn connection.Connection) (description.Server, connection.Connection) {
	...
	for i := 1; i <= maxRetry; i++ {
		...
		now := time.Now()
		isMaster, err := (&connection.CommandIsMaster{}).RoundTrip(ctx, conn)
		if err != nil {
			saved = err
			conn.Close()
			conn = nil
