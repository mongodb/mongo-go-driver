...
func New(ctx context.Context, address addr.Addr, opts ...Option) (Connection, error) {
	...
	if cfg.configurer != nil {
		conn, err = cfg.configurer.Configure(ctx, c)
		if err != nil {
			return nil, err
		}
	}
...
func (c *connection) initialize(ctx context.Context, appName string) error {
	// TODO(skriptble): This should be pipelined together so it's only a single roundtrip.
	isMasterRes, err := (&command.IsMaster{Client: clientDoc(appName)}).RoundTrip(ctx, c)
	if err != nil {
		return err
	}

	buildInfoRes, err := (&command.BuildInfo{}).RoundTrip(ctx, c)
	if err != nil {
		return err
	}
	...
	getLastErrorRes, err := (&command.GetLastError{}).RoundTrip(ctx, c)
	if err != nil {
		return nil
	}
