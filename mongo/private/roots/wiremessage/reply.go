...
func (r Reply) AppendWireMessage(b []byte) ([]byte, error) {
	var err error
	switch r.MsgHeader.MessageLength {
	case 0:
...
func (r *Reply) UnmarshalWireMessage(b []byte) error {
	...
	for pos < len(b) {
		if pos+4 > len(b) {
			return errors.New("invalid OP_REPLY: Malformed documents")
		}
		size := int(readInt32(b, int32(pos)))
		r.Documents = append(r.Documents, bson.Reader(b[pos:pos+size]))
		pos += size
	}
