...
func (q Query) AppendWireMessage(b []byte) ([]byte, error) {
	var err error
	switch q.MsgHeader.MessageLength {
	case 0:
...
func (q *Query) UnmarshalWireMessage(b []byte) error {
	...
	if q.MsgHeader.MessageLength < 36 {
		return errors.New("invalid OP_QUERY: header length too small")
	}
	if len(b) < int(q.MsgHeader.MessageLength) {
		return errors.New("invalid OP_QUERY: []byte too small")
	}
	...
	if pos < len(b) {
		if pos+4 > len(b) {
			return errors.New("invalid OP_QUERY: Malformed Query")
		}
		size := int(readInt32(b, int32(pos)))
		q.Query = bson.Reader(b[pos : pos+size])
		pos += size
	}
	if pos < len(b) {
		if pos+4 > len(b) {
			return errors.New("invalid OP_QUERY: Malformed ReturnFieldsSelector")
		}
		size := int(readInt32(b, int32(pos)))
		q.ReturnFieldsSelector = bson.Reader(b[pos : pos+size])
	}
