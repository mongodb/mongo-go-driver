...
func (a *MongoDBX509Authenticator) Auth(ctx context.Context, c connection.Connection) error {
	authRequestDoc := bson.NewDocument(
		bson.EC.Int32("authenticate", 1),
		bson.EC.String("mechanism", MongoDBX509),
	)

	if !c.Description().Version.AtLeast(3, 4) {
	...
	_, err := authCmd.RoundTrip(ctx, desc, c)
	if err != nil {
		return err
	}
