...
func TestMongoDBCRAuthenticator_Fails(t *testing.T) {
	...
	getNonceReply := msgtest.CreateCommandReply(bson.NewDocument(2).Append(
		bson.C.Int32("ok", 1),
		bson.C.String("nonce", "2375531c32080ae8"),
	))

	authenticateReply := msgtest.CreateCommandReply(bson.NewDocument(1).Append(bson.C.Int32("ok", 0)))
...
func TestMongoDBCRAuthenticator_Succeeds(t *testing.T) {
	...
	authenticateReply := msgtest.CreateCommandReply(bson.NewDocument(1).Append(bson.C.Int32("ok", 1)))
	...
	if !reflect.DeepEqual(getNonceRequest.Query, bson.NewDocument(1).Append(bson.C.Int32("getnonce", 1))) {
		t.Fatalf("getnonce command was incorrect: %v", getNonceRequest.Query)
	}

	authenticateRequest := conn.Sent[1].(*msg.Query)
	expectedAuthenticateDoc := bson.NewDocument(4).Append(
