...
func TestExecuteCommand_Valid(t *testing.T) {
	...
	conn.ResponseQ = append(conn.ResponseQ, msgtest.CreateCommandReply(bson.NewDocument(1).Append(bson.C.Int32("ok", 1))))

	var result okResp
	_, err := ExecuteCommand(context.Background(), conn, &msg.Query{}, &result)
...
func TestExecuteCommand_Error_writing_to_connection(t *testing.T) {
	...
	_, err := ExecuteCommand(context.Background(), conn, &msg.Query{}, nil)
...
func TestExecuteCommand_Error_from_multiple_requests(t *testing.T) {
	...
	reply := msgtest.CreateCommandReply(bson.NewDocument(1).Append(bson.C.Int32("ok", 1)))
	...
	_, err := ExecuteCommands(context.Background(), conn, []msg.Request{&msg.Query{}, &msg.Query{}}, []interface{}{nil, nil})
...
func TestExecuteCommand_QueryFailure_flag_with_document(t *testing.T) {
	...
	conn := &conntest.MockConnection{}
	reply := msgtest.CreateCommandReply(bson.NewDocument(1).Append(bson.C.Boolean("error", true)))
	reply.ResponseFlags = msg.QueryFailure
	conn.ResponseQ = append(conn.ResponseQ, reply)
...
func TestExecuteCommand_OK_field_is_false(t *testing.T) {
	...
	conn := &conntest.MockConnection{}
	reply := msgtest.CreateCommandReply(bson.NewDocument(1).Append(bson.C.Int32("funny", 0)))
	conn.ResponseQ = append(conn.ResponseQ, reply)
	...
	conn = &conntest.MockConnection{}
	reply = msgtest.CreateCommandReply(bson.NewDocument(1).Append(bson.C.Int32("ok", 0)))
	conn.ResponseQ = append(conn.ResponseQ, reply)
	...
	conn = &conntest.MockConnection{}
	reply = msgtest.CreateCommandReply(bson.NewDocument(1).Append(bson.C.Int32("ok", 0), bson.C.String("errmsg", "weird command was invalid")))
	conn.ResponseQ = append(conn.ResponseQ, reply)
