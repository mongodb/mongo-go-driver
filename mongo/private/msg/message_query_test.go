...
func TestWrapWithMeta(t *testing.T) {
	req := NewCommand(10, "admin", true, bson.NewDocument(1).Append(bson.C.Int32("a", 1))).(*Query)

	AddMeta(req, map[string]*bson.Document{
		"$readPreference": bson.NewDocument(1).Append(
			bson.C.String("mode", "secondary"),
		),
	})

	expected, err := bson.NewDocument(2).Append(
