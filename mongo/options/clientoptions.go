...
type ClientOptions struct {
	...
	Err error // holds any errors that occur during processing
}
...
// ApplyURI parses the provided connection string and sets the values and options from
// accordingly.
//
// If an error occurs while parsing the uri, the Err field will be set with that error.
func (c *ClientOptions) ApplyURI(uri string) *ClientOptions {
	...
	if cs.ConnectSet {
		single := cs.Connect == connstring.SingleConnect
		c.Single = &single
	}
	...
	if cs.SSL {
		...
		if cs.SSLClientCertificateKeyFileSet {
			...
			if c.Auth != nil && c.Auth.AuthMechanism == "MONGODB-X509" && c.Auth.Username == "" {
...
// SetDatabase sets the default authentication database.
func (c *ClientOptions) SetDatabase(db string) *ClientOptions {
	c.Database = &db
...
// SetPassword specifies the password to be used during authentication.
func (c *ClientOptions) SetPassword(pwd string) *ClientOptions {
	c.Password = &pwd
	return c
}
...
// SetUsername sets the username used during authentication.
