...
func TestClientOptions(t *testing.T) {
	...
	t.Run("ApplyURI", func(t *testing.T) {
		baseClient := func() *ClientOptions {
			return Client().SetAuth(Credential{AuthSource: "admin"}).SetHosts([]string{"localhost"})
		}
		testCases := []struct {
			name   string
			uri    string
			...
			},
			{
				...
				&ClientOptions{
					...
					Hosts: []string{"localhost"}, Auth: &Credential{AuthSource: "admin"},
				},
			},
			{
			...
			},
			{
				...
				baseClient().SetAuth(Credential{AuthSource: "admin", Username: "foo", Password: "bar"}),
			},
			{
				"Connect",
