...
func (c *Client) configureCrypt(opts *options.AutoEncryptionOptions) error {
	...
	for k, v := range opts.SchemaMap {
		schema, err := transformBsoncoreDocumentMapAllowed(c.registry, v)
		if err != nil {
			return err
		}
		cryptSchemaMap[k] = schema
	}
	kmsProviders, err := transformBsoncoreDocumentMapAllowed(c.registry, opts.KmsProviders)
	if err != nil {
...
func (c *Client) ListDatabases(ctx context.Context, filter interface{}, opts ...*options.ListDatabasesOptions) (ListDatabasesResult, error) {
	...
	filterDoc, err := transformBsoncoreDocumentMapAllowed(c.registry, filter)
	if err != nil {
		return ListDatabasesResult{}, err
	}
