...
func (c *Client) endSessions(ctx context.Context) {
	...
	_ = operation.NewEndSessions().Session(sess).SessionIDs(idArray).ClusterClock(c.clock).Database("admin").
		Deployment(c.topology).ServerSelector(selector).CommandMonitor(c.monitor).Execute(ctx)
