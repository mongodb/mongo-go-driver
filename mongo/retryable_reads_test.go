	...
	t.Run(testFileName, func(t *testing.T) {
		...
		for _, test := range testFile.Tests {
			t.Run(test.Description, func(t *testing.T) {
				runRetryReadsTest(t, &test, &testFile)
			})
		}
	})
}

func runRetryReadsTest(t *testing.T, test *retryReadTest, testFile *retryReadTestFile) {
	coll, cleanup := setUpTest(t, test, testFile)
	defer cleanup()
...
func setUpTest(t *testing.T, test *retryReadTest, testFile *retryReadTestFile) (*Collection, func()) {
	...
	client := createMonitoredClient(t, retryReadsMonitor)
	if test.ClientOptions != nil {
		client.retryReads = test.ClientOptions.RetryReads
	}
	...
	if len(docsToInsert) > 0 {
		...
		_, err = wMajColl.InsertMany(ctx, docsToInsert)
...
func checkRetryExpectations(t *testing.T, expectations []expectation) {
	for _, expectation := range expectations {
		...
		for {
			...
			// check to make sure that evt is not a setup event on the monitored client, but is actually from the test
			if _, err := evt.Command.LookupErr("dropDatabase"); err == nil {
				continue
			}
			if _, err := evt.Command.LookupErr("insert"); err == nil {
				continue
			}
			if _, err := evt.Command.LookupErr("configureFailPoint"); err == nil {
				continue
			}
			break
		}
