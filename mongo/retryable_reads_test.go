...
func TestRetryableReadsSpec(t *testing.T) {
	...
	copts := options.Client().ApplyURI(cs.String()).SetMonitor(retryReadsMonitor)
	retryOnClient, err := NewClient(copts)
...
func runRetryReadsTestFile(t *testing.T, testFileName string, retryOnClient, retryOffClient *Client) {
