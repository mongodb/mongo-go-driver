...
func TestClientSideEncryptionProse(t *testing.T) {
	...
	kvClientOptions := options.Client().ApplyURI(connStr)
	...
	t.Run("data key and double encryption", func(t *testing.T) {
		...
		ceo := options.ClientEncryption().SetKmsProviders(kmsProviders).SetKeyVaultNamespace(kvNamespace).
			SetKeyVaultClientOptions(kvClientOptions)
		cpt := setup(t, aeo, ceo)
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				if tc.externalVault {
					...
					ceo.SetKeyVaultClientOptions(externalKvOpts)
				}
				cpt := setup(t, aeo, ceo)
				defer teardown(t, cpt)
...
func setup(t *testing.T, aeo *options.AutoEncryptionOptions, ceo *options.ClientEncryptionOptions) cseProseTest {
	cpt := cseProseTest{}
	var err error
	...
	if ceo != nil {
		cpt.clientEnc, err = NewClientEncryption(ctx, ceo)
