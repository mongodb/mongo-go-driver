...
type ChangeStreamOptions struct {
	...
	MaxAwaitTimeMS *int64         // The maximum amount of time for the server to wait on new documents to satisfy a change stream query
	ResumeAfter    *bson.Document // Specifies the logical starting point for the new change stream
}
...
// SetMaxAwaitTimeMS specifies the maximum amount of time for the server to wait on new documents to satisfy a change stream query
func (cso *ChangeStreamOptions) SetMaxAwaitTimeMS(i int64) *ChangeStreamOptions {
	cso.MaxAwaitTimeMS = &i
	return cso
}

// SetResumeAfter specifies the logical starting point for the new change stream
func (cso *ChangeStreamOptions) SetResumeAfter(d bson.Document) *ChangeStreamOptions {
	cso.ResumeAfter = &d
	return cso
}

// ToChangeStreamOptions combines the argued ChangeStreamOptions into a single ChangeStreamOptions in a last-one-wins fashion
func ToChangeStreamOptions(opts ...*ChangeStreamOptions) *ChangeStreamOptions {
	csOpts := ChangeStream()
	for _, cso := range opts {
		...
		if cso.MaxAwaitTimeMS != nil {
			csOpts.MaxAwaitTimeMS = cso.MaxAwaitTimeMS
		}
