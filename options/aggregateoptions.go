...
type AggregateOptions struct {
	...
	MaxTimeMS                *int64      // The maximum amount of time to allow the query to run
...
// SetCollations specifies a collation
func (ao *AggregateOptions) SetCollation(c Collation) *AggregateOptions {
	ao.Collation = &c
	return ao
}

// SetMaxTimeMS specifies the maximum amount of time to allow the query to run
func (ao *AggregateOptions) SetMaxTimeMS(i int64) *AggregateOptions {
	ao.MaxTimeMS = &i
	return ao
}
...
func (ao *AggregateOptions) SetMaxAwaitTimeMS(i int64) *AggregateOptions {
	ao.MaxAwaitTimeMS = &i
	return ao
}
...
// ToAggregateOptions combines the argued AggregateOptions into a single AggregateOptions in a last-one-wins fashion
func ToAggregateOptions(opts ...*AggregateOptions) *AggregateOptions {
	aggOpts := Aggregate()
	for _, ao := range opts {
		...
		if ao.MaxTimeMS != nil {
			aggOpts.MaxTimeMS = ao.MaxTimeMS
		}
		if ao.MaxAwaitTimeMS != nil {
			aggOpts.MaxAwaitTimeMS = ao.MaxAwaitTimeMS
		}
		if ao.Comment != nil {
