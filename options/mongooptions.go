...
func (co *Collation) ToDocument() bsonx.Doc {
	doc := bsonx.Doc{}
	if co.Locale != "" {
		doc = append(doc, bsonx.Elem{"locale", bson.String(co.Locale)})
	}
	if co.CaseLevel {
		doc = append(doc, bsonx.Elem{"caseLevel", bson.Boolean(true)})
	}
	if co.CaseFirst != "" {
		doc = append(doc, bsonx.Elem{"caseFirst", bson.String(co.CaseFirst)})
	}
	if co.Strength != 0 {
		doc = append(doc, bsonx.Elem{"strength", bson.Int32(int32(co.Strength))})
	}
	if co.NumericOrdering {
		doc = append(doc, bsonx.Elem{"numericOrdering", bson.Boolean(true)})
	}
	if co.Alternate != "" {
		doc = append(doc, bsonx.Elem{"alternate", bson.String(co.Alternate)})
	}
	if co.MaxVariable != "" {
		doc = append(doc, bsonx.Elem{"maxVariable", bson.String(co.MaxVariable)})
	}
	if co.Backwards {
		doc = append(doc, bsonx.Elem{"backwards", bson.Boolean(true)})
	}
	return doc
}
