...
function compile_check {
  ...
  GOWORK=off ${GC} build -buildmode=plugin

  # Check build with tags.
  GOWORK=off ${GC} build $BUILD_TAGS ./...

  # Check build with various architectures.
  GOWORK=off GOOS=linux GOARCH=386 ${GC} build ./...
  GOWORK=off GOOS=linux GOARCH=arm ${GC} build ./...
  GOWORK=off GOOS=linux GOARCH=arm64 ${GC} build ./...
  GOWORK=off GOOS=linux GOARCH=amd64 ${GC} build ./...
  GOWORK=off GOOS=linux GOARCH=ppc64le ${GC} build ./...
  GOWORK=off GOOS=linux GOARCH=s390x ${GC} build ./...
