		...
		b.Run(bm.name, func(b *testing.B) {
			...
			b.RunParallel(func(p *testing.PB) {
				for p.Next() {
					...
					_, err := coll.InsertOne(context.Background(), bson.D{{"text", t}})
					if err != nil {
			...
			b.RunParallel(func(p *testing.PB) {
				for p.Next() {
					...
					_, err := coll.BulkWrite(context.Background(), []mongo.WriteModel{
						mongo.NewInsertOneModel().SetDocument(bson.D{{"text", t}}),
						mongo.NewInsertOneModel().SetDocument(bson.D{{"text", t}}),
						mongo.NewInsertOneModel().SetDocument(bson.D{{"text", t}}),
						mongo.NewInsertOneModel().SetDocument(bson.D{{"text", t}}),
					})
...
func BenchmarkClientRead(b *testing.B) {
	...
	for _, bm := range benchmarks {
		b.Run(bm.name, func(b *testing.B) {
			...
			_, err = coll.InsertOne(context.Background(), bson.D{{"_id", 0}, {"text", text}})
			if err != nil {
				b.Fatalf("error inserting one document: %v", err)
			}
			_, err = coll.InsertOne(context.Background(), bson.D{{"_id", 1}, {"text", "hello"}})
			if err != nil {
				b.Fatalf("error inserting one document: %v", err)
			}
			...
			b.RunParallel(func(p *testing.PB) {
				for p.Next() {
					var res bson.D
					err := coll.FindOne(context.Background(), bson.M{"_id": random.Int() % 2}).Decode(&res)
					if err != nil {
