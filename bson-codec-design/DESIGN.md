...
functionality of `Marshal` and `Unmarshal` remains unchanged. There is a similar set of functions
for marshaling to and unmarshaling from `*bson.Document` instances.
...
#### Encoder and Decoder
The `Encoder` and `Decoder` types can serialize and deserialize BSON and Extended JSON and marshal
and unmarshal `*bson.Document` instances. The serialization or marshaling type is determined by the
specific reader or writer provided to the decoder when constructed or when it is reset. The `Reset`
method allows the user to change the writer or reader used during the encoding or decoding process.
This allows `Encoder` and `Decoder` instances to be pooled.

#### MarshalDocument and UnmarshalDocument
While a user can create a `ValueReader` or `ValueWriter` for a `*bson.Document` this is an extra
step that doesn't requires extra code on the user's part and also doesn't allow the reuse of
`ValueReader` and `ValueWriter` instances. To help improve usability and add reuse of `ValueReader`
and `ValueWriter` instances, the `MarshalDocument` and `UnmarshalDocument` methods can be used.
These two functions will use a pool to lower the pressure of allocation for applications that do a
lot of marshalling to and unmarshalling from `*bson.Document`s.
