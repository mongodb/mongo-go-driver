...
Go 1.26 or higher is required to run the driver test suite.  We use [task](https://taskfile.dev/) as our task runner.
...
Code should compile and tests should pass under all Go versions which the driver currently supports. Currently, the Go Driver supports a minimum version of Go 1.19 and requires Go 1.26 for development. Please run the following `Taskfile` targets to validate your changes:
