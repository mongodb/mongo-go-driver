...
func AutoCreateIndexes(t *testing.T, keys []string) {
	...
	cmd := command.CreateIndexes{
		...
		Indexes: bson.Arr{bsonx.Document(indexes)},
	}
	id, _ := uuid.New()
...
func DropCollection(t *testing.T, dbname, colname string) {
	cmd := command.Write{DB: dbname, Command: bsonx.Doc{{"drop", bson.String(colname)}}}
	id, _ := uuid.New()
...
func autoDropDB(t *testing.T, topo *topology.Topology) {
	cmd := command.Write{DB: DBName(t), Command: bsonx.Doc{{"dropDatabase", bson.Int32(1)}}}
	id, _ := uuid.New()
	_, err := dispatch.Write(
		context.Background(),
		cmd,
