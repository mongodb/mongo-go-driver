...
type skipCase struct {
	...
	// minServerVersion is the minimum server version for which the skipCase
...
// the test".
var skipTests = map[string][]skipCase{
	...
	"CSOT test not implemented": {
		{
			tests: []string{
				...
				"TestUnifiedSpec/client-side-operations-timeout/tests/sessions-override-timeoutMS.json/timeoutMS_applied_to_withTransaction",
...
// TB is the subset of testing.T used by CheckSkip.
type TB interface {
	Name() string
...
func CheckSkip(t TB, opts ...Option) {
	skipOpts := options{}
	for _, apply := range opts {
		apply(&skipOpts)
	}

	for reason, cases := range skipTests {
		for _, sc := range cases {
			...
			if topo := skipOpts.topology; topo != "" && len(sc.topologies) > 0 {
				if !slices.Contains(sc.topologies, topo) {
					continue
				}
				topoMsg = fmt.Sprintf(" (skip on topologies: %s, current: %s)", strings.Join(sc.topologies, ","), topo)
			}

			// Don't skip if the server version is specified and the current server
			// version is less than the minimum server version for the test case.
			if minVersion := skipOpts.serverVersion; minVersion != "" && sc.minServerVersion != "" {
				if compareServerVersions(minVersion, sc.minServerVersion) < 0 {
					continue
				}
