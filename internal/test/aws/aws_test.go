...
func (p *staticAWSProvider) Retrieve(_ context.Context) (options.AWSCredentials, error) {
	return p.creds, nil
}
...
func (p *trackingAWSProvider) Retrieve(ctx context.Context) (options.AWSCredentials, error) {
	p.called++
...
func TestAWSCustomCredentialProviderPrecedence(t *testing.T) {
	...
	// Populate the custom provider with the valid inline credentials from the URI
	// so that auth actually succeeds via the provider. The bogus AWS_ACCESS_KEY_ID
	// and AWS_SECRET_ACCESS_KEY env vars set above would fail if the driver fell
	// back to the environment-variable provider, so a successful query proves the
	// custom provider took precedence.
	uriOpts := options.Client().ApplyURI(rawURI)
	creds := options.AWSCredentials{
		AccessKeyID:     uriOpts.Auth.Username,
		SecretAccessKey: uriOpts.Auth.Password,
	}
	if uriOpts.Auth.AuthMechanismProperties != nil {
		creds.SessionToken = uriOpts.Auth.AuthMechanismProperties["AWS_SESSION_TOKEN"]
	}
	tracking := &trackingAWSProvider{inner: &staticAWSProvider{creds: creds}}

	// SetAuth overrides any inline credentials that ApplyURI may have parsed from the
	// URI, so the chain is: custom provider -> env-var provider.
	client, err := mongo.Connect(
		options.Client().
			ApplyURI(rawURI).
			SetAuth(options.Credential{
