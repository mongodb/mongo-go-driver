...
func TestCompileCheck(t *testing.T) {
	...
	// Initialize the module and point it at the local driver source.
	...
	// Set the go directive to the minimum version the driver claims (first
	// version in our test list), then tidy under that same toolchain so the
	// generated go.mod is consistent for every version in the matrix. Tidying
	// under a newer toolchain would leave the module needing a re-tidy when
	// built with the minimum version.
	_ = execGo(t, container, nil, "mod", "edit", "-go="+goVersions[0])
	_ = execGo(t, container, &goExecConfig{version: goVersions[0]}, "mod", "tidy")
